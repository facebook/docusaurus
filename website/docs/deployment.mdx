...
- Jamstack providers, e.g. [Netlify](#deploying-to-netlify) and [Vercel](#deploying-to-vercel). We will use Netlify as reference, but the same reasoning can apply to other providers.
...
1. **How much resource (person-hours, money) am I willing to invest in this?**
   ...
   - Jamstack providers can help you set up a working website in almost no time and offers features like server-side redirects that are easily configurable. Build time can be limited—you would need to pay in order to get more build time quota. Check the pricing page of your provider for details.
   - The GitHub pages deployment workflow can be tedious to set up. (Evidence: compare the length of [Deploying to Netlify](#deploying-to-netlify) and [Deploying to GitHub pages](#deploying-to-github-pages)!) However, this service (including build and deployment) is always free, and we have detailed instructions to help you make it work.
2. **How many server-side configuration would I need?**
...
3. **Do I have needs to cooperate?**
   - Self-hosted services can achieve the same effect as Netlify, but with much more heavy-lifting.
   - Netlify and Vercel have deploy previews for every pull request, which is useful for a team to review others' work before doing a production build.
   - GitHub pages cannot do deploy previews in a non-convoluted way. One repo can only be associated with one site deployment.

There isn't a silver bullet. You need to weigh your needs and resources before making your choice.
