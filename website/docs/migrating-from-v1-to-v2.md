...
If you are migrating your Docusaurus v1 website, and there are pending documentation pull requests, you can temporarily keep the `/docs` folder to its original place, to avoid producing conflits.
...
Also, you have to apply the site layout around each page. The Layout component takes metadata props.
...
const MyPage = (props) => {
  ...
  const {tagline, customFields = {myHomepageDescription}} = context;

  return (
    <Layout permalink="/" title={tagline} description={myHomepageDescription}>
      <div>...</div>
    </Layout>
  );
};
...
You can migrate to ES6 imports/exports.
