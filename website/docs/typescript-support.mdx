...
Since TypeScript 6.0, the `types` compiler option defaults to `[]`, and `@types/*` packages are not loaded automatically anymore. Your site needs [Node.js types](https://www.npmjs.com/package/@types/node) to type-check:

- Node.js code, such as `docusaurus.config.ts` or `sidebars.ts`
- Node.js globals that the bundler makes available to your client code, such as `require()` or `process.env`

We recommend installing the `@types/node` version matching the minimum Node.js version your site runs on.
