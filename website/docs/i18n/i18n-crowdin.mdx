...
You can integrate Docusaurus with the **tools and SaaS of your choice**, as long as you put the **translation files at the correct place**.
...
Temporarily, you can hardcode your personal token in `crowdin.yml` with `api_token: 'MY-HARDCODED-TOKEN'`.
...
Use the `Hide String` feature to make sure **translators don't translate frontmatter fields** like `id`, `slug`, `tags`...
...
We will configure the CI to **download the Crowdin translations at build time**.

Add `website/i18n` to `.gitignore`.

:::note

You can also commit the `i18n` folder and set-up Crowdin to [send translation pull-requests to your GitHub repository](https://support.crowdin.com/github-integration/)
