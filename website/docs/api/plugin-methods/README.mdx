...
export default async function myPlugin(context, opts) {
  return {
    ...
    async postStart(props) {
      // Called after docusaurus <start> finishes
      console.log('Development server started successfully');
    },

    afterDevServer(app, server) {
      // Called after the development server starts
      // Useful for adding middleware or custom routes
      console.log('Development server is running');
    },

    beforeDevServer(app, server) {
      // Called before the development server starts
      // Useful for configuring server options or adding middleware
