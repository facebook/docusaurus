...
You can link a navbar item to the first document of a given sidebar item, without having to specify a given doc ID.

If the first item in the sidebar is of type `'category'`, the first child item of that sidebar will be selected, unless a `link` object is given for that sidebar. If a category's `link` is specified, that link's path will be linked to this navbar item instead.

For files ordering in an `'autogenerated'` sidebar, please refer to the [autogenerated sidebar documentation](https://docusaurus.io/docs/sidebar#sidebar-item-autogenerated).
...
| `sidebarId` | `'string'` | **Required** | The ID of the sidebar item that this item is linked to. |
