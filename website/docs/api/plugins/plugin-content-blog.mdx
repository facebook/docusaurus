...
| `created` | `FrontMatterCreated` | `undefined` | Allows overriding the created author/date. Date can be any [parsable date string](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Date/parse). |
...
```ts
type FrontMatterCreated = {date?: string; author?: string};
