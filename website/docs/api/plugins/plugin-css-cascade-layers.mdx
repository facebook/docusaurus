...
This plugin is mostly designed to be used internally by the classic preset through the [Docusaurus `v4.useCssCascadeLayers` future flag](../docusaurus.config.js.mdx#future), although it can also be used as a standalone plugin. Please [let us know here](https://github.com/facebook/docusaurus/pull/11142) if you have a use case for it and help us design an API that makes sense for the future of Docusaurus.
...
If you use the preset `@docusaurus/preset-classic`, this plugin is automatically configured for you with the `siteConfig.v4.useCssCascadeLayers` future flag.
