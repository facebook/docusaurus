...
- `experimental_viewTransitions`: Enables smooth animated page transitions during client-side navigation using the [View Transition API](https://developer.mozilla.org/en-US/docs/Web/API/View_Transition_API). Defaults to `false`. Respects `prefers-reduced-motion`, feature-detects browser support, and falls back to instant navigation when unavailable. See the [View Transitions guide](../guides/view-transitions.mdx) for setup and customization.
