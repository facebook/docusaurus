...
If your site uses multiple locales, it is possible to provide a distinct `url` for each locale thanks to the [`siteConfig.i18n.localeConfigs[<locale>].url`](#i18n) attribute. This makes it possible to [deploy a localized Docusaurus site over multiple domains](../i18n/i18n-tutorial.mdx#multi-domain-deployment).
...
type MeasureImageFunction = (params: {
  imagePath: string; // absolute path of the resolved local image
}) =>
...
export default {
  markdown: {
    ...
    hooks: {
      ...
      // measureImage: async ({imagePath}) => ({width: 16, height: 16}),
    },
  },
};
```
...
| `hooks.onUnusedMarkdownDirectives` | `ReportingSeverity \| OnUnusedMarkdownDirectivesFunction` | `'warn'` | Hook to customize the behavior when encountering an unused [Markdown directive](https://github.com/remarkjs/remark-directive). Also accepts a callback function, giving you access to the raw `Directive` AST nodes. |
| `hooks.measureImage` | `MeasureImageFunction` | `undefined` | Override how local Markdown images are measured at build time (`width`/`height` attributes). The default measures SVG via a local parser and common raster formats (JPEG/PNG/GIF/WebP/AVIF/HEIF) via [`image-dimensions`](https://github.com/sindresorhus/image-dimensions). Return `null` to skip emitting dimensions. Formats previously handled only by `image-size` (BMP/ICO/TIFF/PSD/JXL/ICNS) are no longer auto-measured unless you provide this hook. |
