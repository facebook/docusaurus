...
Docusaurus supports themed images by `ThemedImage` component included in the theme. It can be used, if you want to switch the image source based on the current theme.
...
`ThemedImage` component can also be used as regular `img` tag, when instead of `sources` you provide the single image source via `src` property.
