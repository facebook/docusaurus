...
- `fallback` (optional): JSX to render on the server (Node.js) and until React hydration completes. It is also displayed while the `children` are suspended, because `<BrowserOnly>` renders its own `<Suspense>` boundary.
