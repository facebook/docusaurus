...
1. You might have missed the previous mention earlier, but the syntax of all configurable fields is available at [**`docusaurus.config.js` API reference**](api/docusaurus.config.js.mdx). Read it. It can be helpful.
2. Check that variables or constants are indeed being set correctly.  Somewhere.  Now, double check them again.
3. Using a customized template?  Did you try its own `docusaurus.config.js` that includes defaults for the common options?
4. ...
