...
The initial template `docusaurus.config.js` uses the `const config = { ... }` and it is equivalent to `module.exports = { ... }` shown throughout the examples.
There are many equivalent ways to declare your Docusaurus configuration.
