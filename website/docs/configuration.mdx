...
Here is a verbose example using most of the syntax that is available ([docusaurus.config.js API](api/docusaurus.config.js.mdx), JS, Node, constants, variables, etc.):
...
Since a config file is Javascript then import/require can be utilized to pull in libraries to help.  (For advanced users, just know that we )

// TODO: link for nodejs import/require.  And answer question about interoperability for CommonJS and ECMAScript modules.
