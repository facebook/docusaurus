...
Docusaurus generate an author's page listing all authors with their respective blog posts if the author have `page: true`.

By default the permalink of the author is the author's key, this ensure unique permalink between authors. You can customize the permalink for the author, see example below.

Blog posts must reference the author's key to be displayed on the author's page. Inline authors are not supported on the author's page.
...
slorber: # author key
  ...
  url: https://sebastienlorber.com
  image_url: https://github.com/slorber.png
  page: true # Enable the authors page for this author (route will be /authors/slorber)
  socials:
    x: sebastienlorber
    github: slorber
...
You can configure the route base path for the authors page using the [`authorsBasePath`](api/plugins/plugin-content-blog.mdx#authorsBasePath).
