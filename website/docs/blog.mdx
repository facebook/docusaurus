...
If you want to render additional author information (`twitter`, etc.), object notation is more preferable than string fields (prefer `author: {twitter: 'id'}` to `author_twitter: 'id'`), because the former will be packed into the blog post's `metadata.authors`, while the latter will stay in the front matter.
