...
Blog post tags can be declared directly inside the front matter:
...
For regular blog post tags, it can be tedious to maintain tags inlined in each blog post.

It is possible to declare those tags globally in a configuration file:

```yml title="website/blog/tags.yml"
tag:
  hello: null
  docusaurus-v2:
    label: Docusaurus v2
    permalink: /docusaurus-v2
    description: Docusaurus v2 related posts
```
