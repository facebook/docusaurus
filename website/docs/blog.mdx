...
There are a variety of patterns of file/folder structure from which Docusaurus can extract the date. For example, posts can be placed into folders named by year, and the files inside be named from the month onwards. (See "Supported date extraction patterns" below.) This flexible pattern extraction method is a powerful way to manage large numbers of posts in your folder. 
