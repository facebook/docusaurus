...
Use the `tags` front matter field use tags declared in `tags.yml`. Tags are a way to categorize your blog posts.

### Inline tags {#inline-tags}

Tags are considered inline when they are not defined in a global configuration file. They will be reported as per the `onInlineTags` configuration in `docusaurus.config.js`.

Blog post tags can be declared directly inside the front matter in different ways:

```md title="my-blog-post.md"
---
tags: [hello, docusaurus, {label: 'Open Source', permalink: '/open-source'}]
---
```
...
Global tags are declared in a configuration file which then tag's key can be referenced in the blog post front matter.

See [the API documentation](./api/plugins/plugin-content-blog.mdx#markdown-front-matter) to learn about `tags.yml`.
