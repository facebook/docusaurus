...
You can make parts of a code block collapsible by adding `sample-start` and `sample-end` comments.

:::note

Code blocks with highlighted ranges specified in the title like ` ```js {1,3-4} ` will not be parsed at all. If you want both highlighting and collapsibility, use inline comments instead.
...
export default function Component({defaultData}) {
