...
When adding a custom language definition to Docusaurus you do not need to add the language to the `additionalLanguages` config array. Adding the language import in `prism-include-languages.js` is sufficient and adding custom languages to `additionalLanguages` will result in an error.
