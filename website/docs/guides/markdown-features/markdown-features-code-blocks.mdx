...
Display CLI commands in both NPM and Yarn is a very common need, for example:
...
And then use it by adding the `npm2yarn` key:
...
Using the `{sync: true}` option would make all tab choices synced (see [Syncing tab choices](https://docusaurus.io/docs/next/markdown-features/tabs#syncing-tab-choices)). Because the choice is stored under the same namespace `npm2yarn`, different `npm2yarn` plugin instances would also sync their choices.
