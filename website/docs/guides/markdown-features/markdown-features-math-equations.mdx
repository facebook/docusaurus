...
The latest version of `rehype-katex` (starting from `v6.0.0`) has moved to ES Modules, which Docusaurus doesn't officially support yet. However, it is possible to import `rehype-katex` dynamically, using an async config creator.

```js title="docusaurus.config.js"
async function createConfig() {
  // ES Modules are imported with `import()` instead of `require()`, and is imported asynchronously
  // highlight-next-line
