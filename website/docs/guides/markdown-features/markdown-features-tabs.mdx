...
Alternatively, you can put all the values in the `Tabs` props, and then only declare the respective `value` in each `TabItem`. The `values` and `defaultValue` configured in `Tabs` take precedence over the `label` and `default` props inlined in each `TabItem`.
