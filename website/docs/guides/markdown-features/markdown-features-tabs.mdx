...
<Tabs
  ...
  <TabItem value="orange" label="Orange 2" default>
    This is an orange 🍊
  </TabItem>
  <TabItem value="banana" label="Banana 2">
    This is a banana 🍌
  </TabItem>
</Tabs>
```
...
<BrowserWindow>
  <Tabs
    ...
    <TabItem value="orange" label="Orange 2" default>This is an orange 🍊</TabItem>
...
The first tab is displayed by default, and to override this behavior, you can specify a default tab by add `default` to one of the tab items. You can also set the `defaultValue` prop in the `Tabs` component to the label value of your choice. For example, in the example above, setting `default` for the `value="apple"` tab forces it to be open by default.

If `defaultValue` is provided for the `Tabs`, but it refers to an non-existing value, only the tab headings appear until the user clicks on a tab.
