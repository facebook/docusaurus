...
You can also customize each tab heading independently, by using the `attributes` field. You can either pass them through the `values` prop in `Tabs` or props of each `TabItem`—just like `label`.
