...
The `values` and `defaultValue` configured in `Tabs` take precedence over the `label` and `default` props inlined in each `TabItem`.

If no `label` is configured for a tab item, the `value` is used by default.
