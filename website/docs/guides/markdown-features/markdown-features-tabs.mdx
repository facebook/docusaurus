...
:::warning The only valid children for `Tabs` are `TabItem` elements.
...
<Tabs
  ...
  <TabItem value="orange">This is an orange 🍊</TabItem>
  <TabItem value="banana">This is a banana 🍌</TabItem>
  :::tip This is a tip admonition in an invalid position. :::
</Tabs>
```

This will trigger the build error:

> Bad `<Tabs>` child `<Admonition>`: all children of the `<Tabs>` component should be `<TabItem>`.

The valid usage would be:
...
:::tip
  This is a tip admonition in an invalid position.
:::
```

You could also place the admonition _inside_ a `TabItem` element.
