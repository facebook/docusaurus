...
:::note SVGs in CommonJS require and Import statement

To use SVGs with CommonJS require and Import statements you need to override the default `svg-loader` behavior by prepending `!!url-loader!` to the asset path. For example:

```jsx
<img
  src={
    require('!!url-loader!./assets/docusaurus-asset-example-banner.svg').default
  }
  alt="Example banner in SVG"
/>
```

```jsx
import mySvgImageUrl from '!!url-loader!./assets/docusaurus-asset-example-banner.svg';

<img src={mySvgImageUrl} alt="Example banner in SVG" />;
```
