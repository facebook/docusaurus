...
If you use the doc shorthand or [autogenerated](#sidebar-item-autogenerated) sidebar, you would lose the ability to customize the sidebar label through item definition. You can, however, use the `sidebar_label` markdown front matter within that doc, which has a higher precedence over the `label` key in `SidebarItemDoc`.
...
Use the [**shorthand syntax**](#category-shorthand) when you don't need **category options**:
...
Note how the autogenerate source directories themselves don't become categories: only the items they contain do. This is what we mean by saying "sidebar slice".
