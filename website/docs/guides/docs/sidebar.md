...
If you didn't provide a sidebars file, Docusaurus [automatically generates a sidebar](#sidebar-item-autogenerated) for you, by using the filesystem structure of the `docs` folder:
...
A sidebar at its crux is a hierarchy of categories, doc links, and other hyperlinks. For example:
...
This is a sidebars file that exports one sidebar, called `mySidebar`. It has three top-level items: two categories and one external link. Within each category, there are a few doc links. A sidebars file can contain **multiple sidebar objects**, identified by their object keys.
...
For typical sidebar items without much customization, you can express them more concisely with **shorthand syntaxes**. There are two parts to this: doc shorthands and category shorthands.

### Doc shorthand {#doc-shorthand}

An item with type `doc` can be simply a string:
...
This permits us to simplify the example to:
...
Note how the two consecutive category shorthands are compressed into one object. This syntax generates a **sidebar slice**: you shouldn't see that object within the braces as one bulk item—each entry is a separate items which will be unwrapped and spliced together with the rest of the items (in this case, the "Learn more" link) to form the final sidebar level. Sidebar slices are also important when discussing [autogenerated sidebars](#sidebar-item-autogenerated).
...
When you add a item of doc Y to sidebar X, it creates a two-way binding: sidebar X contains a link to doc Y, and when browsing doc Y, sidebar X will be displayed. But sometimes, we want to break either implicit binding:

1. _How do I generate a link to doc Y in sidebar X without making sidebar X displayed on Y?_ For example, when I include a link to the doc Y in multiple sidebars as in the example above, and I want to explicitly tell Docusaurus to display one sidebar?
...
`commonDoc` used to be associated to two sidebars, but when we break its association with `tutorialSidebar` with `displayThisSidebar: false`, Docusaurus can be certain that `apiSidebar` should be displayed.
...
The pagination label by default is the sidebar label, which then defaults to the title. You can use the front matter `pagination_label` to customize how this doc appears in the pagination.
...
- **\[Deprecated]Ref**: link to a doc page, without associating it with the sidebar
...
If you use the doc shorthand or [autogenerated](#sidebar-item-autogenerated) sidebar, you would lose the ability to customize the sidebar label, which defaults to the title, through the item definition. You can, however, use the `sidebar_label` markdown front matter within that doc, which has a higher precedence over the `label` key in `SidebarItemDoc`.
...
An `autogenerated` item is converted by Docusaurus to a **sidebar slice** (also discussed in [category shorthands](#category-shorthand)): a list of items of type `doc` and `category`, so you can splice **multiple `autogenerated` items** from multiple directories and interleave them with regular sidebar items in one sidebar level.
...
Note how the autogenerate source directories themselves don't become categories: only the items they contain matter. This is what we mean by saying "sidebar slice".
...
Previously, you would provide metadata to sidebar items when you declare them; for autogenerated, Docusaurus would read them from the respective files.
