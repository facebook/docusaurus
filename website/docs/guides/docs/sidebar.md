...
Now, although the link to `commonDoc` is still generated in the `tutorialSidebar` sidebar, when browsing `commonDoc`, only `apiSidebar` can be possibly displayed.
