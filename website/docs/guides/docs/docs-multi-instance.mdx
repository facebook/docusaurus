...
With large documentation sets, rebuilding can often require additional work to verify and stage the set that did not change.
...
For both use cases, you can use docs plugin instance for each content set. Using the use case examples, docs one plugin for `docs` and one for `community` or one plugin for Android and one for iOS mobile SDK documentation.
