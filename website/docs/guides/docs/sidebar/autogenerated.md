...
**For docs**: use additional front matter. The `label` and `className` attributes are declared in front matter as `sidebar_label` and `sidebar_class_name`. Position and custom props can also be defined the same way, via `sidebar_position` and `sidebar_custom_props` front matter.
