...
To address this, we’ve hardened our publishing pipeline by adopting **[npm Trusted Publishing](https://docs.npmjs.com/trusted-publishers)** for both stable ([#11819](https://github.com/facebook/docusaurus/pull/11819)) and canary releases ([#11712](https://github.com/facebook/docusaurus/pull/11712)), and by mitigating cache-poisoning risks in the publish process ([#11795](https://github.com/facebook/docusaurus/pull/11795)). All releases now go through a single `publish.yml` GitHub Actions workflow using short-lived tokens, replacing previous flows that relied on maintainer machines or long-lived tokens that could be stolen.

We also reduced the risk of cache poisoning attacks ([#11795](https://github.com/facebook/docusaurus/pull/11795)) affecting the publish process. All our releases will now be published from a single `publish.yml` GitHub Action workflow using a short-lived token. Previously, we were publishing releases from a maintainer's computer or a workflow, using long-lived tokens that could be stolen.

This significantly reduces the risk of shipping compromised packages. Every release now includes verifiable provenance, with a transparency log showing exactly how and when it was published.
