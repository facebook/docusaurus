...
In [#11874](https://github.com/facebook/docusaurus/pull/11874), we introduced a new security workflow that looks for suspicious dependency updates published on npm that affect our official packages and their transitive dependencies. It runs every day and on each pull-request.

:::caution Not Bulletproof

Using semver dependency ranges, we can't guarantee a 100% secure supply chain, since any new npm publish in our dependency graph can introduce a vulnerability. This workflow can only let us know as soon as possible when a serious vulnerability affects our community. Ultimately, you have to secure your site with additional measures. The bare minimum is to use a lockfile and upgrade dependencies carefully.
...
- Blocks usage of GitHub repository and tarball url dependencies, using [pnpm `blockExoticSubdeps`](https://pnpm.io/settings#blockexoticsubdeps)
- Blocks packages that downgrade their trust level, using [pnpm `trustPolicy: no-downgrade`](https://pnpm.io/settings#trustpolicy)
- Do similar checks on our own monorepo, to protect Docusaurus maintainers and contributors

:::tip How to secure your

Read the [npm security best practices](https://github.com/lirantal/npm-security-best-practices) guide to learn how to further secure your site.
