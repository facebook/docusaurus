...
This is how we choose to implement the workflow, but you are free to choose alternative tools for each step if you want.
...
This module contains implementation details that we choose to hide to give you a more concise overview of the overall workflow.
...
![Docusaurus + Argos monorepo example screenshot](./img/docusaurus-argos-example-repo-screenshot.png)
...
I am a firm believer that visual regression testing is currently underused in the frontend ecosystem. Taking full page screenshots is a low-hanging fruit easy to set up, and can help you catch a new class of bugs that your regular test suite would usually miss. It doesn't only shine for package upgrades, but also for large refactorings that are not supposed to change the UI.

The tools we choose are implementation details of this visual regression workflow, and you are free to adopt alternative tools if you want. At Docusaurus, we choose [Argos](https://argos-ci.com/): it works well for us, and has [free](https://argos-ci.com/pricing) and [open source](https://argos-ci.com/docs/open-source) plans.

In case you don't mind storing large screenshots in Git, you can also try the free [Playwright Visual Comparisons](https://playwright.dev/docs/test-snapshots) and check visual differences with `npx playwright show-report`. However, using a dedicated external tool is often more convenient to use.
