...
The tools we choose are implementation details of this visual regression workflow, and you are free to adopt alternative tools if you want. At Docusaurus, we choose [Argos](https://argos-ci.com/): it works well for us, and offers [free](https://argos-ci.com/pricing) and [open source](https://argos-ci.com/docs/open-source) plans.

In case you don't mind storing large screenshots in Git, you can also try the free, self-hosted [Playwright Visual Comparisons](https://playwright.dev/docs/test-snapshots) and check visual differences with `npx playwright show-report`. However, using a dedicated external tool is more convenient.
