...
The [slorber/docusaurus-argos-example](https://github.com/slorber/docusaurus-argos-example) shows a complete example of implementing this workflow on a newly initialized Docusaurus v2 site, using a Yarn monorepo.
...
Taking full page screenshots is a **low-hanging fruit** easy to set up, and can help you **catch a new class of bugs** that your regular test suite would miss. This technique shines not only for npm package upgrades, but also for **any kind of refactoring** that shouldn't change the user interface.
