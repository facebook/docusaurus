...
This article introduces a **visual regression testing** workflow based on [**GitHub Actions**](https://github.com/features/actions), [**Playwright**](https://playwright.dev/) and [**Argos**](https://argos-ci.com/). It is not directly coupled to Docusaurus nor React, and can be adapted to work with other frontend applications and frameworks.
...
For this reason we recommend injecting in pages an extra stylesheet to hide the problematic elements. You will probably need to add new CSS rules to this base stylesheet, according to flaky elements found on your own site. Read [Argos - About flaky tests docs](https://argos-ci.com/docs/about-flaky) for details.
...
We recommend reading the official documentation of each tool used in this workflow.
...
The [slorber/docusaurus-argos-example](https://github.com/slorber/docusaurus-argos-example) shows a complete example of implementing this workflow on a newly initialized Docusaurus site, using a Yarn monorepo.
...
External tools can be expensive, but generally offer free plans with a good amount of screenshots. You can keep your screenshot consumption to a minimum by implementing a few tips described below.
...
For the Docusaurus website, we decided it was not worth it to take screenshot of versioned documentation pages:
...
Implementing [GitHub Actions concurrency groups](https://docs.github.com/en/actions/using-jobs/using-concurrency) will prevent successive commits to trigger multiple useless workflow runs. The workflow will only be executed for tha last commit, and previous commits will be canceled automatically.
...
For the Docusaurus website, we choose to only run Argos when the pull-request has a certain GitHub label:
...
Taking full page screenshots is a low-hanging fruit easy to set up, and can help you catch a new class of bugs that your regular test suite would usually miss. This technique shines not only for npm package upgrades, but also for any kind of refactoring that shouldn't change the user interface.
