...
答案是：做一个 MDX 插件！ MDX 内置了一个[插件系统](https://mdxjs.com/advanced/plugins/)，可以用来自定义 Markdown 文件是如何被如何解析并转换到 JSX 的。 MDX 插件有三种典型使用场景：
...
- **[Rehype](https://github.com/rehypejs/rehype/)**: 处理超文本 AST。

:::tip

你可以用插件来为项目中最常用的 JSX 元素创建简短的语法。 我们提供的[告示语法](./markdown-features-admonitions.mdx)也是由一个 Remark 插件生成的。你可以根据自己的用途仿照着来做。

:::

## 默认插件 {#default-plugins}

Docusaurus 会在处理 Markdown 时插入[一些默认的 Remark 插件](https://github.com/facebook/docusaurus/tree/main/packages/docusaurus-mdx-loader/src/remark)。 这些插件会：
...
每个 MDX 插件一般是一个 npm 包，所以你可以像其他包一样通过 npm 安装。 以[公式插件](./markdown-features-math-equations.mdx)为例。
...
接下来，导入你的插件并在 `docusaurus.config.js` 的插件选项或预设配置中添加它们 ：
...
<details>
  <summary>正在使用 [**CommonJS**](https://nodejs.org/api/modules.html#modules-commonjs-modules) 的配置文件？</summary>
