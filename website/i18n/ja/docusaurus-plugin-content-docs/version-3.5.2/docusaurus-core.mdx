...
The `fallback` prop is a callback, and **not a React functional component**. このコールバック内ではReactフックを使用できません。

:::

### `<Head/>` {#head}

この再利用可能なReactコンポーネントは、ドキュメントのヘッドに対するすべての変更を管理します。 プレーンなHTMLタグを受け取り、プレーンなHTMLタグを出力するため、初心者にも優しいです。 It is a wrapper around [React Helmet](https://github.com/nfl/react-helmet).
