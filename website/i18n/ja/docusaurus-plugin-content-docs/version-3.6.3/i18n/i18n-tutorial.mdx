...
The `docusaurus write-translations` command only does **static analysis** of your code. 実際にサイトを動かすわけではありません。 Therefore, dynamic messages can't be extracted, as the message is an _expression_, not a _string_:
