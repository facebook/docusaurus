---
sidebar_label: クライアントAPI
---

# DocusaurusのクライアントAPI

Docusaurusは、サイト構築の際に役立つAPIをクライアントに提供しています。

## コンポーネント {#components}

### `<ErrorBoundary />` {#errorboundary}

このコンポーネントは[Reactのerror boundary](https://reactjs.org/docs/error-boundaries.html)を作成します。

例外を投げる可能性のあるコンポーネントをくるみ、アプリ全体を機能停止させるのではなく、例外が発生したときにフォールバックを表示するために使用します。
...
const SafeComponent = () => (
  <ErrorBoundary
    fallback={({error, tryAgain}) => (
      <div>
        <p>エラー発生のためコンポーネントが機能停止しました：{error.message}</p>
        <button onClick={tryAgain}>やり直す！</button>
      </div>
    )}>
...
動作の様子を見たい場合は、クリックしてみてください→ <ErrorBoundaryTestButton/>

:::

:::info

Docusaurusはこのコンポーネントを使用して、テーマのレイアウトやアプリ全体のエラーを検出します。

:::

:::note

このコンポーネントは、ビルド時のエラーをキャッチせず、ステートフルなReactコンポーネントを使用するときに起こりうるクライアント側のレンダー・エラーのみを防ぎます。

:::

#### プロパティ {#errorboundary-props}

- `fallback`：JSX要素を返す省略可能なレンダーコールバック。 `error`（捕捉されたエラー）・`tryAgain`（コンポーネント内のエラーをリセットし、再レンダリングを試みる関数（`() => void`）コールバック）という2つの属性を持つオブジェクトを受け取ります。 存在しない場合、`@theme/Error`が代わりにレンダリングされます。 `@theme/Error` is used for the error boundaries wrapping the site, above the layout.

:::warning

`fallback`プロパティはコールバックであり、**Reactの関数コンポーネントではありません**。 このコールバック内ではReactフックを使用できません。

:::

### `<Head/>` {#head}

この再利用可能なReactコンポーネントは、ドキュメントのヘッドに対するすべての変更を管理します。 プレーンなHTMLタグを受け取り、プレーンなHTMLタグを出力するため、初心者にも優しいです。 これは[React Helmet](https://github.com/nfl/react-helmet)のラッパーです。
...
入れ子または後から追加されたコンポーネントは、重複した内容を上書きします。
...
The `siteConfig` object only contains **serializable values** (values that are preserved after `JSON.stringify()`). 関数や正規表現などはクライアント側では失われます。
