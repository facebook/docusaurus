...
このセクションでは、スタイルシートによるスタイルの変更について記述しています。 より高度なカスタマイズ（DOM構造・Reactコードなど）については、[swizzlingガイド](./swizzling.mdx)を参照してください。
...
`@docusaurus/preset-classic`を使っている場合は、そのオプションに独自に作成した CSS ファイル(たとえば`/src/css/custom.css`など)を指定して、グローバルスタイルとして読み込ませることができます。
...
- **テーマクラス名** These class names are listed exhaustively in [the next subsection](#theme-class-names). They don't have any default properties. You should always prioritize targeting those stable class names in your custom CSS.
...
`docusaurus-theme`クエリ文字列パラメータから直接Docusaurusテーマを初期化することができます。
