---
description: Docusaurusを使えば、すぐにフル機能のブログをデプロイできます。
---
...
ブログ機能を使えば、フル機能のブログをすぐに設置することができます。

:::info

[ブログプラグインの API リファレンスドキュメント](./api/plugins/plugin-content-blog.mdx) で、オプションの完全な一覧表を確認してください。

:::

## 初期設定 {#initial-setup}

サイトにブログを設置するには、まず`blog`というディレクトリを作成してください。

そして、`docusaurus.config.js`の中に、あなたのブログへのアイテムリンクを追加します。
...
ブログで公開するには、blogディレクトリ内にMarkdownファイルを作成します。
...
[フロントマター](./guides/markdown-features/markdown-features-intro.mdx#front-matter)は、ブログ記事に著者情報などのメタデータを追加するのに便利ですが、Docusaurusはフロントマターがなくても必要なメタデータをすべて推測することができます。 指定可能な全てのフィールドについては、[APIドキュメント](api/plugins/plugin-content-blog.mdx#markdown-front-matter)を参照してください。

## ブログリスト {#blog-list}

The blog's index page (by default, it is at `/blog`) is the _blog list page_, where all blog posts are collectively displayed.

ブログ記事内の`<!-truncate-->`マーカーを使用して、公開されたすべてのブログ記事を表示する際にサマリーとして表示される内容を表します。 `<!--truncate-->`より上の部分は、要約の一部となります。 トランケートマーカーより上の部分はスタンドアロンでレンダリング可能なMarkdownでなければならないことに注意してください。 例：

```md title="website/blog/my-post.md" {7}
---
title: Markdown blog truncation example
---
これらはすべてブログ投稿のサマリーの一部になります。
...
`.mdx` 拡張子を使用するファイルには、 [MDX](https://mdxjs.com/) コメント `{/* truncate */}` を代わりに使用します:
...
デフォルトでは、各ブログ一覧ページに10個の投稿が表示されます。 しかし、プラグインの設定で `postsPerPage` オプションでページネーションをカスタマイズできます。 `postsPerPage: 'ALL'`を設定すると、ページネーションは無効になり、最初のページにすべての投稿が表示されます。 また、ブログ一覧ページにメタディスクリプションを追加することで、SEOを向上させることもできます:
