...
[サイトをローカライズする](../i18n/i18n-introduction.mdx)ための国際化（i18n）設定オブジェクト。
...
Docusaurusが[重複したルート](/guides/creating-pages.mdx#duplicate-routes)を検出した時の動作。
...
ナビゲーションバーやフッターのようなサイトUIをカスタマイズするための[テーマ設定](./themes/theme-configuration.mdx) オブジェクト。
...
`PluginModule`の型定義については[プラグインメソッドリファレンス](./plugin-methods/README.mdx)を参照してください。
...
HTMLの`<head>`タグ内に挿入されるタグの配列。 値は、`tagName`と`attribute`という2つのプロパティを含むオブジェクトでなければなりません。 `tagName` は、作成されるタグを決める文字列でなければなりません。（例：`"link"`） `attributes` は、属性名からその値に対応するマップ（連想配列）でなければなりません。

- 型：`{ tagName: string; attributes: Object; }[]`
...
これは、生成されたHTMLでは`<link rel="icon" href="img/docusaurus.png" />` となります。
...
サイト全体で読み込まれる[クライアントモジュール](../advanced/client.mdx#client-modules) の配列。
