...
内部の webpack の設定を変更できます。 返り値が JavaScript オブジェクトであれば、[`webpack-merge`](https://github.com/survivejs/webpack-merge) を用いることで最終的な設定にマージされます。 If it is a function, it will be called and receive `config` as the first argument and an `isServer` flag as the second argument.
...
`configureWebpack` は、クライアント/サーバのビルドに応じて生成された `config` で呼び出されます。 これは、マージする際のベースとなる設定として扱われることができます。

### `isServer` {#isServer}

`configureWebpack` はサーバとクライアントの両方のビルド時に呼び出されます。 `isServer` は、サーバービルドにおいては `true` を、クライアントビルドにおいては `false` を受け取ります。
...
### 設定をマージする際の指針 {#merge-strategy}

プラグインにおける Webpack 設定の一部を [webpack-merge](https://github.com/survivejs/webpack-merge) を使ってグローバルな Webpack 設定とマージします。

すなわち、マージ方針を指定することができます。 例えば、webpack の rule を appended ではなく prepended にしたい場合は以下のようになります：
...
詳細については、 [webpack-merge strategy doc](https://github.com/survivejs/webpack-merge#merging-with-strategies) を参照してください。
