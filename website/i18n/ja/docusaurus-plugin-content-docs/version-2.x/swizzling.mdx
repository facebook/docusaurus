...
Docusaurus provides an convenient **interactive CLI** to swizzle components. 通常は、次のコマンドだけ覚えておけば十分です。
...
`docusaurus swizzle` is only an automated way to help you swizzle the component. You can also create the `src/theme/SomeComponent.js` file manually, and Docusaurus will [resolve it](./advanced/client.mdx#theme-aliases). このコマンドの中に魔法などありません！
...
Ejecting an [**unsafe**](#what-is-safe-to-swizzle) component can sometimes lead to copying a large amount of internal code, which you now have to maintain yourself. 受け取ったプロパティや使用する内部テーマAPIが変更された場合、カスタマイズした内容を移行する必要があるため、Docusaurusのアップグレードをより困難にすることがあります。
