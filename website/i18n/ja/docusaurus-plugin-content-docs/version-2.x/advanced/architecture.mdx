...
- すべてのプラグインライフサイクルメソッドは Node で実行されます。 Therefore, until we support ES Modules in our codebase, plugin source code must be provided as CommonJS that can be `require`'d.
