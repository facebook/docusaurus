...
既存のプロジェクトのドキュメント作成にDocusaurusを使用している場合、モノレポが解決策になるかもしれません。 モノレポは、類似のプロジェクト間で依存関係を共有することを可能にします。 例えば、あなたのウェブサイトでは、リリースされたバージョンに依存する代わりに、ローカルパッケージを使用して最新の機能を導入することができます。また、あなたの貢献者は、機能を実装する際にドキュメントを更新することができます。 モノレポのフォルダー構造の例を以下に示します。
