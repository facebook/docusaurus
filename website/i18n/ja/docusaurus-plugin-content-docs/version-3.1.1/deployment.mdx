...
- 🟢 Jamstackのプロバイダーを利用すると、ほとんど時間をかけずにウェブサイトを立ち上げることができ、サーバーサイドリダイレクトなどの機能を使えて、簡単に設定することができます。 Many providers offer generous build-time quotas even for free plans that you would almost never exceed. However, free plans have limits, and you would need to pay once you hit those limits. 詳しくは、プロバイダーの価格ページをご覧ください。
...
通常、公開プロセスには2つのリポジトリ (少なくとも2つのブランチ) が関与しています。ソースファイルを含むブランチと、GitHub Pagesで配信するビルド出力を含むブランチです。 In the following tutorial, they will be referred to as **"source"** and **"deployment"**, respectively.
