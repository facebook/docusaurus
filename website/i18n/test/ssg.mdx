...
_Le rendu côté serveur (SSR)_ et la _génération de site statique (SSG)_ peuvent être des concepts différents, mais nous les utilisons de manière interchangeable.
...
## Compréhension de SSR {#understanding-ssr}
...
Il est important de réaliser que les enfants de `<BrowserOnly>` ne sont pas un élément JSX, mais une fonction _qui renvoie_ un élément. C'est une décision de conception. Considérons ce code :
...
function MyComponent() {
  useEffect(() => {
    ...
    console.log("Je suis maintenant dans le navigateur");
  }, []);
