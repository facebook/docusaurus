...
| `editUrl`                     | <code>string \| [EditUrlFn](#EditUrlFn)</code>                                                                    | `undefined`                                | 사이트를 편집하기 위한 Base URL입니다. The final URL is computed by `editUrl + relativePostPath`. 옵션 사용 시 각 파일에 대한 세밀한 제어를 할 수 있습니다. 해당 필드를 설정하지 않으면 편집 링크가 비활성화됩니다. |
...
| `blogSidebarCount`            | <code>number \| 'ALL'</code>                                                                                      | `5`                                        | 블로그 사이드바에 표시할 블로그 게시물 수입니다. `'ALL'` to show all blog posts; `0` to disable.                                                                             |
...
| `archiveBasePath`             | <code>string \| null</code>                                                                                       | `'archive'`                                | 블로그 아카이브 섹션에 대한 URL 라우트 Will be appended to `routeBasePath`. **DO NOT** include a trailing slash. Use `null` to disable generation of archive.          |
...
| `postsPerPage`                | <code>number \| 'ALL'</code>                                                                                      | `10`                                       | 목록 페이지에서 페이지 당 표시할 게시물 수입니다. Use `'ALL'` to display all posts on one listing page.                                                                      |
...
| `truncateMarker`              | `RegExp`                                                                                                          | `/<!--\s*(truncate)\s*-->/`                | 요약이 끝나는 곳을 표시하는 Truncate marker                                                                                                                         |
...
| `feedOptions`                 | _See below_                                                                                                       | `{type: ['rss', 'atom']}`                  | 블로그 피드                                                                                                                                                  |
| `feedOptions.type`            | <code>[FeedType](#FeedType) \| [FeedType](#FeedType)[] \| 'all' \| null</code>                                    | **Required**                               | 생성할 피드 유형입니다. Use `null` to disable generation.                                                                                                         |
...
| `feedOptions.language`        | `string` (See [documentation](http://www.w3.org/TR/REC-html40/struct/dirlang.html#langcodes) for possible values) | `undefined`                                | 피드 언어 메타데이터                                                                                                                                             |
...
| 옵션명                      | 타입         | 기본값             | 설명                                                                                                                                                                                                                   |
| ------------------------ | ---------- | --------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `authors`                | `Authors`  | `undefined`     | 블로그 게시물 작성자들(또는 단독 작성자) 목록 Read the [`authors` guide](../../blog.mdx#blog-post-authors) for more explanations. Prefer `authors` over the `author_*` front matter fields, even for single author blog posts.          |
...
| `draft`                  | `boolean`  | `false`         | 블로그 게시물이 작업 중임을 나타내는 부울 플래그입니다. 비공개 설정 게시물은 개발 상태에서만 확인할 수 있습니다.                                                                                                                                                     |
