...
이 설정은 모든 [메인 테마](./overview.mdx)에 적용됩니다.

## 공통 {#common}

### 컬러 모드 {#color-mode---dark-mode}
...
`respectPrefersColorScheme: true`로 설정한 경우 `defaultMode` 설정값은 사용자 시스템 환경 설정에 따라 재정의됩니다.

하나의 색상 모드만 지원하려면 사용자 시스템 환경 설정에 영향을 받지 않게 해야 합니다.

:::

### 메타 정보 이미지 {#meta-image}

`og:image`나 `twitter:image`처럼 메타 태그에서 사용할 기본 이미지를 설정합니다.
...
| `logo`         | _아래를 참조하세요._              | `undefined` | 로고 오브젝트 사용자 지정                       |
...
로고는 [static 디렉터리](static-assets.mdx)에 가져다 놓을 수 있습니다. 로고 URL은 기본적으로 여러분의 사이트 기본 URL이 설정됩니다. 로고 URL을 따로 설정할 수는 있지만 외부 링크인 경우에는 새로운 탭에서 열리게 됩니다. 또한 로고 링크의 target 속성값을 재정의할 수 있습니다. 메인 웹 사이트의 하위 디렉터리에서 문서 웹 사이트를 서비스하는 경우에 유용한 기능입니다. 이런 경우 메인 웹 사이트를 새로운 탭에서 열어주는 로고 링크가 필요하지 않을 수도 있습니다.
...
| `width`     | <code>string \| number</code> | `undefined`                                                 | `width` 속성을 설정합니다.                                      |
| `height`    | <code>string \| number</code> | `undefined`                                                 | `height` 속성을 설정합니다.                                     |
...
리액트 라우터는 자동으로 링크에 대해서 활성 링크 스타일을 적용합니다. 하지만 필요에 따라 `activeBasePath`를 사용할 수 있습니다. 여러 다른 경로의 링크를 활성화해야 하는 경우(예를 들면 같은 사이드바 아래 여러 문서 디렉터리가 포함된 경우) `activeBaseRegex`를 사용할 수 있습니다. `activeBaseRegex`는 `activeBasePath`보다 유연한 대안이며 우선적으로 적용됩니다. 도큐사우루스에서는 현재 URL에 정규식으로 적용해 구문 분석을 처리합니다.
...
| `label`                | `string`                  | **필수**        | 아이템을 표시하는 이름입니다.                                                      |
| `html`                 | `string`                  | 선택사항          | `label`과 같지만 텍스트 콘텐츠 대신 HTML을 렌더링합니다.                                 |
| `to`                   | `string`                  | **필수**        | 웹 사이트 내에서 탐색을 위해 사용하는 클라이언트 라우팅입니다. 해당 값 앞에 baseUrl이 자동으로 추가됩니다.      |
| `href`                 | `string`                  | **필수**        | 웹 사이트 외부 탐색을 위해 사용하는 전체 페이지 탐색입니다. **`to` 또는 `href` 중 하나만 사용해야 합니다.** |
| `prependBaseUrlToHref` | `boolean`                 | `false`       | baseUrl을 `href` 값 앞에 추가합니다.                                           |
...
| `activeBaseRegex`      | `string`                  | `undefined`   | 필요한 경우 `activeBasePath`를 대체해 사용할 수 있습니다.                              |
...
#### 메뉴바 드롭다운 {#navbar-dropdown}

`dropdown` 유형의 메뉴바 아이템은 추가적으로 `items` 필드를 내부 배열 형태로 가질 수 있습니다.

메뉴바 드롭다운 아이템은 아래와 같이 **"링크 기능을 지원하는" 아이템 유형**만 허용합니다.

- [메뉴바 링크](#navbar-link)
- [메뉴바 문서 링크](#navbar-doc-link)
- [메뉴바 문서 버전](#navbar-docs-version)
- [메뉴바 문서 사이드바](#navbar-doc-sidebar)
- [메뉴바 사용자 지정 HTML](#navbar-with-custom-html)

드롭다운 기본 아이템도 클릭할 수 있는 링크이므로 해당 아이템은 [일반적인 메뉴바 링크](#navbar-link)의 모든 속성을 가질 수 있습니다.
...
| `label`    | `string`                  | **필수**   | 아이템을 표시하는 이름입니다.           |
| `items`    | <code>[LinkLikeItem](#navbar-dropdown)[]</code> | **필수**   | 드롭다운에 포함될 아이템입니다.          |
| `position` | <code>'left' \| 'right'</code> | `'left'` | 메뉴바에서 아이템이 표시되는 위치를 설정합니다. |
...
특정 문서에 대한 링크를 만들고 싶은 경우 특정 메뉴바 아이템 타입을 설정하면 `docId`를 기준으로 문서 링크를 만듭니다. 같은 사이드바 문서를 탐색하고 있다면 `navbar__link--active` 클래스를 얻게 됩니다.
...
| 옵션명            | 타입                        | 기본값         | 설명                         |
| -------------- | ------------------------- | ----------- | -------------------------- |
| `type`         | `'doc'`                   | **필수**      | 항목 유형을 문서 링크로 설정합니다.       |
| `docId`        | `string`                  | **필수**      | 아이템에서 링크로 연결되는 문서의 ID입니다.  |
...
#### 사이드바에 연결된 메뉴바 {#navbar-doc-sidebar}
...
| 옵션명            | 타입                        | 기본값                 | 설명                                |
| -------------- | ------------------------- | ------------------- | --------------------------------- |
| `type`         | `'docSidebar'`            | **필수**              | 메뉴바 아이템의 유형을 사이드바 첫 번째 문서로 설정합니다. |
| `sidebarId`    | `string`                  | **필수**              | 아이템에서 링크로 연결되는 사이드바 ID입니다.        |
...
#### 메뉴바 문서 버전 드롭다운 {#navbar-docs-version-dropdown}
...
| 옵션명                           | 타입                         | 기본값         | 설명                                                                                                                                                          |
| ----------------------------- | -------------------------- | ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `type`                        | `'docsVersionDropdown'`    | **필수**      | 항목 유형을 문서 버전 드롭다운으로 설정합니다.                                                                                                                                  |
| `position`                    | <code>'left' \| 'right'</code>  | `'left'`    | 메뉴바에서 아이템이 표시되는 위치를 설정합니다.                                                                                                                                  |
...
| 옵션명            | 타입                         | 기본값          | 설명                         |
| -------------- | -------------------------- | ------------ | -------------------------- |
| `type`         | `'docsVersion'`            | **필수**       | 항목 유형을 문서 버전 링크로 설정합니다.    |
...
#### 메뉴바 로케일 드롭다운 {#navbar-locale-dropdown}

[i18n 기능](../../i18n/i18n-introduction.mdx)을 사용하고 있다면 특정 메뉴바 아이템을 설정해 사이트에서 사용할 수 있는 로케일을 드롭다운 목록에 표시할 수 있습니다.
...
| 옵션명                   | 타입                         | 기본값         | 설명                                          |
| --------------------- | -------------------------- | ----------- | ------------------------------------------- |
| `type`                | `'localeDropdown'`         | **필수**      | 항목 유형을 로케일 드롭다운으로 설정합니다.                    |
| `position`            | <code>'left' \| 'right'</code> | `'left'`    | 메뉴바에서 아이템이 표시되는 위치를 설정합니다.                  |
| `dropdownItemsBefore` | <code>[LinkLikeItem](#navbar-dropdown)[]</code> | `[]`        | 드롭다운 시작 부분에 드롭다운 아이템을 추가합니다.                |
| `dropdownItemsAfter`  | <code>[LinkLikeItem](#navbar-dropdown)[]</code> | `[]`        | 드롭다운 끝 부분에 드롭다운 아이템을 추가합니다.                 |
...
#### 메뉴바 검색 {#navbar-search}

[검색 기능](../../search.mdx)을 사용하고 있다면 메뉴바 오른쪽 끝에 검색바가 표시됩니다.
...
| 옵션명         | 타입                         | 기본값      | 설명                         |
| ----------- | -------------------------- | -------- | -------------------------- |
| `type`      | `'search'`                 | **필수**   | 항목 유형을 검색바로 설정합니다.         |
| `position`  | <code>'left' \| 'right'</code> | `'left'` | 메뉴바에서 아이템이 표시되는 위치를 설정합니다. |
...
| 옵션명         | 타입                         | 기본값      | 설명                               |
| ----------- | -------------------------- | -------- | -------------------------------- |
| `type`      | `'html'`                   | **필수**   | 항목 유형을 HTML 요소로 설정합니다.           |
| `position`  | <code>'left' \| 'right'</code> | `'left'` | 메뉴바에서 아이템이 표시되는 위치를 설정합니다.       |
| `className` | `string`                   | `''`     | 메뉴바 아이템을 위한 사용자 지정 CSS 클래스       |
...
### 메뉴바 자동 숨김 {#auto-hide-sticky-navbar}
...
### 메뉴바 스타일 {#navbar-style}

테마 전환 기능을 비활성화하지 않고도 메뉴바의 스타일을 따로 설정할 수 있습니다. 선택한 스타일은 선택한 테마와 상관없이 항상 적용됩니다.

현재 설정할 수 있는 스타일 옵션은 2가지입니다. `dark`, `primary`(`--ifm-color-primary` 색상에 따라) [인피마 문서](https://infima.dev/docs/components/navbar/)에서 적용될 스타일을 확인해볼 수 있습니다.
...
도큐사우루스에서는 [Prism React Renderer](https://github.com/FormidableLabs/prism-react-renderer)를 사용해 코드 블록 구문 강조를 처리합니다. 모든 설정은 `prism` 오브젝트에서 처리합니다.
...
| `magicComments`   | `MagicCommentConfig[]` | _아래를 참조하세요._ | [매직 주석](../../guides/markdown-features/markdown-features-code-blocks.mdx#custom-magic-comments) 목록입니다. |
...
마크다운 구문을 강조해야 하는 경우 어두운 모드 구문 강조 테마 사용 시 다른 배경색을 설정해야 할 수도 있습니다.  [적용 가이드](../../guides/markdown-features/markdown-features-code-blocks.mdx#line-highlighting)를 참고하세요.

:::

### 기본 언어 {#default-language}

3개의 억음부호(```) 뒤에 언어를 설정하지 않았을 때 코드 블록에서 기본으로 적용할 언어를 설정할 수 있습니다. 유효한 [언어명](https://prismjs.com/#supported-languages)을 설정해주어야 하는 것은 주의해주세요.
...
`themeConfig.footer` 설정에서 바닥글에 로고나 저작권 정보를 추가할 수 있습니다. 로고는 [static 디렉터리](static-assets.mdx)에 가져다 놓을 수 있습니다. 로고 URL은 메뉴바 로고와 같은 방식으로 처리됩니다.
...
| 옵션명         | 타입                         | 기본값         | 설명                                            |
| ----------- | -------------------------- | ----------- | --------------------------------------------- |
| `logo`      | `Logo`                     | `undefined` | 로고 오브젝트 사용자 지정 [메뉴바 로고](#navbar-logo)를 참고하세요. |
...
`themeConfig.footer.links` 설정에서 바닥글에 링크를 추가할 수 있습니다. 바닥글 구성에는 **multi-column footers**와**simple footers** 두 가지 유형이 있습니다.

다중 컬럼 바닥글 링크는 각 컬럼에 대한 `title`과 `FooterItem` 목록이 있습니다.
...
`FooterItem`에서 사용할 수 있는 필드는 아래와 같습니다.
...
| 옵션명     | 타입       | 기본값         | 설명                                                                                 |
| ------- | -------- | ----------- | ---------------------------------------------------------------------------------- |
| `label` | `string` | **필수**      | 링크에 대해 표시할 텍스트입니다.                                                                 |
| `to`    | `string` | **필수**      | 웹 사이트 내에서 탐색을 위해 사용하는 클라이언트 라우팅입니다. 해당 값 앞에 baseUrl이 자동으로 추가됩니다.                   |
| `href`  | `string` | **필수**      | 웹 사이트 외부 탐색을 위해 사용하는 전체 페이지 탐색입니다. **`to` 또는 `href` 중 하나만 사용해야 합니다.**              |
| `html`  | `string` | `undefined` | 단순 링크 대신 HTML 패스스루(pass-through)를 렌더링합니다. `html` 필드를 설정한 경우에는 다른 옵션은 설정하지 않아야 합니다. |
...
`themeConfig.tableOfContents`에서 기본 목차를 설정할 수 있습니다.
...
`useColorMode`를 호출하는 컴포넌트는 `Layout` 컴포넌트의 자식 요소이어야 합니다.
