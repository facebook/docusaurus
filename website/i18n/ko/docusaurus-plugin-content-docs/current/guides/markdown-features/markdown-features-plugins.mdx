...
해결책은 MDX 플러그인을 만드는 겁니다! MDX는 마크다운 파일을 구문분석하고 JSX로 변환하는 방식을 사용자가 지정할 수 있는 내장 [플러그인 시스템](https://mdxjs.com/advanced/plugins/)이 있습니다. MDX 플러그인의 세 가지 일반적인 사용 사례는 다음과 같습니다.

- 기존 [remark plugins](https://github.com/remarkjs/remark/blob/main/doc/plugins.md#list-of-plugins) 또는 [rehype plugins](https://github.com/rehypejs/rehype/blob/main/doc/plugins.md#list-of-plugins)을 사용합니다.
...
- **[Remark](https://github.com/remarkjs/remark/)**: 마크다운 AST를 처리합니다.
- **[Rehype](https://github.com/rehypejs/rehype/)**: 하이퍼텍스트 AST를 처리합니다.

:::tip

플러그인을 사용해 프로젝트에서 가장 일반적으로 사용하는 JSX 요소를 좀 더 짧은 구문으로 만들 수 있습니다. 도큐사우루스에서 사용하는 [준수 사항](./markdown-features-admonitions.mdx) 구문은 Remark 플러그인으로 만들어지며 여러분도 여러분만의 준수 사항을 위한 플러그인을 사용할 수 있습니다.

:::

## 기본 플러그인 {#default-plugins}

도큐사우루스는 마크다운 처리를 위해 [몇 가지 기본 Remark 플러그인](https://github.com/facebook/docusaurus/tree/main/packages/docusaurus-mdx-loader/src/remark)을 사용합니다. 이런 플러그인은 다음 작업을 처리합니다.
...
MDX 플러그인은 npm 패키지 형태로 제공됩니다. npm을 사용해 다른 npm 패키지처럼 설치할 수 있습니다. [math 플러그인](./markdown-features-math-equations.mdx)이라면 아래와 같이 설치합니다.
