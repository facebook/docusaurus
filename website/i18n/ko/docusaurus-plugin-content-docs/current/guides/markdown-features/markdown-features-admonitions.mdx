...
## 프리티어(Prettier) 사용하기 {#usage-with-prettier}

[프리티어](https://prettier.io)를 사용해 마크다운 파일의 포맷을 처리한다면 프리티어가 잘못된 준수 사항 구문으로 자동 수정할 수 있습니다. 이 문제를 피하려면 시작과 종료 지시문 주위에 빈 줄을 추가하세요. 이곳에 사용한 모든 예제에 빈 줄을 사용한 이유이기도 합니다.
...
## 준수 사항 내에서 MDX 사용하기 {#admonitions-with-mdx}
...
## 사용자 지정 준수 사항 {#customizing-admonitions}

**구문분석**과 **렌더링** 2가지 방식으로 준수 사항을 사용자 지정할 수 있습니다.

### 사용자 지정 렌더링 동작 {#customizing-rendering-behavior}

You can customize how each individual admonition type is rendered through [swizzling](../../swizzling.mdx). 간단한 래퍼를 사용해 여러분이 원하는 목표를 달성할 수 있습니다. 예를 들어 아래의 예는 `info` 준수 사항 타입의 경우 아이콘을 변경합니다.
...
### 사용자 지정 구문분석 동작 {#customizing-parsing-behavior}

준수사항은 [Remark 플러그인](./markdown-features-plugins.mdx)으로 구현됐습니다. 이 플러그인은 설정할 수 있도록 설계됐습니다. 특정 콘텐츠 플러그인(docs, blog, pages)의 Remark 플러그인을 사용자 지정하기 위해 `admonitions` 키를 통해 옵션을 전달합니다.
...
`keyword`는 `Admonition` 컴포넌트의 `type` 속성으로 전달됩니다.

### 사용자 지정 준수 사항 타입 컴포넌트 {#custom-admonition-type-components}
