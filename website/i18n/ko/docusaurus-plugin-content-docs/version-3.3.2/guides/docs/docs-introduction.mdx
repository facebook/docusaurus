...
There's also a "blog-only mode" for those who only want to use the blog feature of Docusaurus. 문서 전용 모드와 비슷한 방법을 적용합니다. Follow the setup instructions on [Blog-only mode](../../blog.mdx#blog-only-mode).
