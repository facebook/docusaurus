...
### Configuração do tema {#theme-configuration}
...
### Iniciar seu site {#start-your-site}
...
Agora você pode fazer [deploy](../deployment.mdx) e `build` na solução de hospedagem estática de sua escolha.
...
## Traduzir o seu site {#translate-your-site}
...
A maioria dos sites de documentação são geralmente **estáticos** e não precisam de recursos i18n avançados (**plurais**, **gêneros**, etc.). Use uma biblioteca como [react-intl](https://www.npmjs.com/package/react-intl) para casos de uso mais avançados. Therefore, dynamic messages can't be extracted, as the message is an _expression_, not a _string_:
...
#### Traduzir o blog {#translate-the-blog}
...
#### Traduzir as páginas {#translate-the-pages}
...
## Deploy do seu site {#deploy-your-site}

You can choose to deploy your site under a **single domain** or use **multiple (sub)domains**.

### Implementação de um domínio {#single-domain-deployment}
...
### Deploy multi-domínio {#multi-domain-deployment}
...
No seu [provedor de hospedagem estática](../deployment.mdx):
