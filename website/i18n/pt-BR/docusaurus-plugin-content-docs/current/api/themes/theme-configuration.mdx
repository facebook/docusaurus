...
### Modo de cor {#color-mode---dark-mode}
...
### Barra de Anúncios {#announcement-bar}
...
#### Link da barra de navegação {#navbar-link}
...
#### Lista suspensa da barra de navegação {#navbar-dropdown}
...
- [Link da barra de navegação](#navbar-link)
- [Link de doc na barra de navegação](#navbar-doc-link)
- [Versão dos docs da Navbar](#navbar-docs-version)
- [Navbar doc sidebar](#navbar-doc-sidebar)
- [Navbar with custom HTML](#navbar-with-custom-html)

Note que o item de base suspenso também é um link clicável para que este item possa receber qualquer uma das propriedades de um [link simples da barra de navegação](#navbar-link).
...
#### Lista suspensa de versões de documentos da Navbar {#navbar-docs-version-dropdown}
...
#### Menu suspenso de localidade da Navbar {#navbar-locale-dropdown}
...
#### Pesquisa na Navbar {#navbar-search}
...
### Auto-ocultar a barra de navegação {#auto-hide-sticky-navbar}
...
### Estilo barra de navegação {#navbar-style}
...
Atualmente, existem duas possíveis opções de estilo: `dark` e `primary` (baseado na cor `--ifm-color-primary`). Você pode ver a pré-visualização de estilos na [documentação da Infima](https://infima.dev/docs/components/navbar/).
...
O Docusaurus usa [Prism React Renderer](https://github.com/FormidableLabs/prism-react-renderer) para destacar blocos de código. All configuration are in the `prism` object.
...
Se você usar a linha que destaca a sintaxe Markdown, talvez seja necessário especificar uma cor de destaque diferente para o tema de destaque do modo escuro. Consulte a documentação [para obter orientação](../../guides/markdown-features/markdown-features-code-blocks.mdx#line-highlighting).
...
Pode adicionar o logotipo e um direito autoral ao rodapé através do `themeConfig.footer`. Logo can be placed in [static folder](static-assets.mdx). A URL do logotipo funciona da mesma maneira do logotipo da barra de navegação.
...
### Exemplo de estrutura de sistema de arquivos {#example-file-system-structure}
