...
## Adicionando posts {#adding-posts}
...
A [front matter](./guides/markdown-features/markdown-features-intro.mdx#front-matter) é útil para adicionar mais metadados ao seu post de blog, por exemplo, informações de autor, mas o Docusaurus será capaz de inferir todos os metadados necessários sem o front matter. Para todos os campos possíveis, consulte [a documentação da API](api/plugins/plugin-content-blog.mdx#markdown-front-matter).

## Lista de Blog {#blog-list}

A página de índice do blog (por padrão, está em `/blog`) é a _blog list page_, onde todas as postagens são exibidas coletivamente.

Use the `<!--truncate-->` marker in your blog post to represent what will be shown as the summary when viewing all published blog posts. Anything above `<!--truncate-->` will be part of the summary. Note that the portion above the truncate marker must be standalone renderable Markdown. Por exemplo:
...
## Data da postagem no blog {#blog-post-date}
...
| Pattern                          | Exemplo                                     |
| -------------------------------- | ------------------------------------------- |
| Single file                      | `2021-05-28-meu-blog-post-title.md`         |
...
### Autores globais {#global-authors}
...
Use o callback para todas as suas necessidades de personalização:
...
```md "my-blog-post.md"
---
hide_reading_time: verdadeiro
---

Esta página não irá mais exibir as estatísticas do tempo de leitura!
```
...
### Modo somente blog {#blog-only-mode}
...
Não se esqueça de excluir a página inicial existente em `./src/pages/index.js` ou então haverá um mapeamento de dois arquivos para a mesma rota!
...
Como exemplo, nós hospedamos um segundo blog [aqui](/tests/blog).
