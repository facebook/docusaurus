---
id: code-blocks
description: Manipulando blocos de código no Markdown do Docusaurus
...
Blocos de código são blocos de texto envolvidos por strings de 3 crases (backticks). You may check out [this reference](https://mdxjs.com/docs/) for the specifications of MDX.
...
```
````
Use a meta string de idioma correspondente para o seu bloco de código, e o Docusaurus irá pegar o destaque de sintaxe automaticamente, alimentado por [Prism React Renderer](https://github.com/FormidableLabs/prism-react-renderer).
<BrowserWindow>
...
By default, the Prism [syntax highlighting theme](https://github.com/FormidableLabs/prism-react-renderer#theming) we use is [Palenight](https://github.com/FormidableLabs/prism-react-renderer/blob/master/packages/prism-react-renderer/src/themes/palenight.ts). Você pode alterar isso para outro tema passando o campo `theme` no `prism` como `themeConfig` no seu docusaurus.config.js.
...
Você pode consultar as definições de idioma oficiais do [Prisma](https://github.com/PrismJS/prism/tree/master/components) quando você estiver escrevendo suas próprias definições de idioma.
...
## Destaque de linha {#line-highlighting}

### Destacar com comentários {#highlighting-with-comments}
...
## Editor de código interativo {#interactive-code-editor}
...
## Suporte para blocos de código multi-linguagem {#multi-language-support-code-blocks}
...
```
````
#### Configuração {#npm2yarn-remark-plugin-configuration}
