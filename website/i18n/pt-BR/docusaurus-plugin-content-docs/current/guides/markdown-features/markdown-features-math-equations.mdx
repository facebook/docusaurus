---
id: math-equations
description: Escrevendo Equações Matemáticas LaTeX
...
As equações matemáticas podem ser processadas usando [KaTeX](https://katex.org).
...
\int_{a}^{x} f(t)\,dt$. Então $F$ é contínua e $x$ tal que $f$ é contínua em $x$, $F$ é derivável $x$ com $F'(x)=f(x)$.
