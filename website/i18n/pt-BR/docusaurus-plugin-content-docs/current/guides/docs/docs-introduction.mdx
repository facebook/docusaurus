...
## Modo somente documentos {#docs-only-mode}
...
There's also a "blog-only mode" for those who only want to use the blog feature of Docusaurus. Você pode usar o mesmo método detalhado acima. Follow the setup instructions on [Blog-only mode](../../blog.mdx#blog-only-mode).
