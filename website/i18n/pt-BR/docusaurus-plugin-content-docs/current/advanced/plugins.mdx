...
## Criando plugins {#creating-plugins}

Um plugin é uma função que recebe dois parâmetros: `context` e `options`. It returns a plugin instance object (or a promise). You can create plugins as functions or modules. For more information, refer to the [plugin method references section](../api/plugin-methods/README.mdx).
...
### Definição do módulo {#module-definition}
...
Este é um exemplo inventado: na prática, `@docusaurus/theme-classic` fornece o tema para a documentação, blog e layouts.
...
E se você quiser usar o estilo Bootstrap, você pode alterar o tema com `theme-blog-bootstrap` (outro tema fictício não existente):
