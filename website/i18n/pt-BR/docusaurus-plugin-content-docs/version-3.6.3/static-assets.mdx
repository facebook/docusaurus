...
By default, you are suggested to put these assets in the `static` folder. Every file you put into **that directory will be copied** into the root of the generated `build` folder with the directory hierarchy preserved. Ex. if you add a file named `sun.jpg` to the static folder, it will be copied to `build/sun.jpg`.
