...
5. Crie uma nova pipeline. Define its name, set the trigger mode to `On push`, and select the branch that triggers the pipeline execution.
...
4. In the project page (which looks like `https://dev.azure.com/ORG_NAME/REPO_NAME/_build`), create a new pipeline with the following text. Also, click on edit and add a new environment variable named `GH_TOKEN` with your newly generated token as its value, then `GH_EMAIL` (your email address) and `GH_NAME` (your GitHub username). Certifique-se de marcá-los como secretos. Alternatively, you can also add a file named `azure-pipelines.yml` at your repository root.
...
1. Cria uma conta no Qovery. Visit the [Qovery dashboard](https://console.qovery.com) to create an account if you don't already have one.
2. Create a project.
...
4. Add an application.
   ...
   - Click on **Create**. Depois que o aplicativo for criado:
