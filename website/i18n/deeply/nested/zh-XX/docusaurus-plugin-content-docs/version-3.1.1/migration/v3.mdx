---
slug: /migration/v3
sidebar_label: 到 Docusaurus v3
---

# 升级到 Docusaurus v3

此文档将会帮助你将站点从 Docusaurus v2 升级到 Docusaurus v3。

Docusaurus v3 是一个新的**主要版本**，包含**破坏性更改**，需要你相应地调整你的站点。 我们将在这一过程中提供指导，并提供一些可选项建议。
...
主要的突破变化是从 MDX v1 升级到 MDX v3 。 请阅读 [**MDX v2**](https://mdxjs.com/blog/v2/) 和 [**MDX v3**](https://mdxjs.com/blog/v3/) 的版本说明获取更详细的信息。 MDX will now compile your Markdown content **more strictly** and with **subtle differences**.

:::tip 升级前

在升级以前，我们建议[**为站点升级到 Docusaurus v3 做好准备**](/blog/preparing-your-site-for-docusaurus-v3)。 There are changes that you can already **handle incrementally, under Docusaurus v2**. 这样做将有助于减少最终升级到 Docusaurus v3 所需的工作。

For complex sites, we also recommend to set up [**visual regression tests**](/blog/upgrading-frontend-dependencies-with-confidence-using-visual-regression-testing), a good way to ensure your site stays visually identical. Docusaurus v3 主要是升级依赖关系，预期中不会产生任何视觉变化。
...
升级到 Docusaurus v3 需要升级核心的 Docusaurus 依赖关系(`@docusaurus/name`)，但也需要其他相关的软件包。
...
Upgrading MDX comes with all the breaking changes documented on the [MDX v2](https://mdxjs.com/blog/v2/) and [MDX v3](https://mdxjs.com/blog/v3/) release blog posts. 大多数重大变更来自MDX v2。 The [MDX v2 migration guide](https://mdxjs.com/migrating/v2/) has a section on how to [update MDX files](https://mdxjs.com/migrating/v2/#update-mdx-files) that will be particularly relevant to us. Also make sure to read the [Troubleshooting MDX](https://mdxjs.com/docs/troubleshooting-mdx/) page that can help you interpret common MDX error messages.
...
- [MDX 游乐场 - 当前版本](https://mdxjs.com/playground/)
- [MDX 游乐场 - v1](https://mdx-git-renovate-babel-monorepo-mdx.vercel.app/playground/)
...
- `h1` 的映射 `MDXComponent` 仅用于`# hi` ，而不是用于`<h1>hi</h1>`
- **小写** 自定义元素名称将不再被相应的 `MDXComponent` 组件所替代。
...
{/_ For Chinese translators: you can translate the following Japanese into Chinese. _/}
...
React 18 comes with its own breaking changes that should be relatively easy to handle, depending on the amount of custom React code you created for your site. 官方主题和插件与 React 18 兼容。
...
理论上，您不需要做些什么，您现有的交互代码块应该像以前一样继续工作。

However, this is a new major library version containing breaking changes, and we can't guarantee a strict retro-compatibility. 如果遇到问题，请阅读 [v3](https://github.com/FormidableLabs/react-live/releases/tag/v3.0.0) 和 [v4](https://github.com/FormidableLabs/react-live/releases/tag/v4.0.0) 变更日志。

:::

### remark-emoji v4.0+

Docusaurus v3 将 [`remark-emoji`](https://github.com/rhysd/remark-emoji) 升级到 v4.0+ 。 这一库支持在 Markdown 使用 `:emoji:` 的快捷方式。
...
Please consider **our time is precious**. 为了确保您的支持请求不被忽视，我们敬请您：
