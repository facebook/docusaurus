...
For all tab groups that have the same `groupId`, the possible values do not need to be the same. If one tab group is chosen a value that does not exist in another tab group with the same `groupId`, the tab group with the missing value won't change its tab. 您可以从下面的例子中看到这一点。 尝试选择 Linux，上面的标签组不会改变。
