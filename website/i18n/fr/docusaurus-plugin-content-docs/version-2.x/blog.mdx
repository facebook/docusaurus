...
Consultez la [documentation de référence de l'API du plugin du Blog](./api/plugins/plugin-content-blog.mdx) pour une liste exhaustive des options.
...
Par exemple, créez un fichier depuis `website/blog/2019-09-05-hello-docusaurus-v2.md` :

```md title="website/blog/2019-09-05-hello-docusaurus-v2.md"
---
title: Bienvenue à Docusaurus v2
...
Le [front matter](./guides/markdown-features/markdown-features-intro.mdx#front-matter) est utile pour ajouter des métadonnées supplémentaires à votre article de blog, par exemple des informations sur l'auteur, mais Docusaurus sera capable de déduire toutes les métadonnées nécessaires sans le frontmatter. Pour connaître tous les champs possibles, consultez [la documentation de l'API](api/plugins/plugin-content-blog.mdx#markdown-front-matter).
...
```md
---
title: Exemple de troncature
---

Tous ces éléments feront partie du résumé de l'article de blog.

Même ceci.

<!--truncate-->

Mais tout ce qui se passera à partir d'ici ne le sera pas.
...
Par défaut, 10 articles sont affichés sur chaque page de la liste du blog, mais vous pouvez contrôler la pagination avec l'option `postsPerPage` dans la configuration du plugin. Si vous définissez `postsPerPage: 'ALL'`, la pagination sera désactivée et tous les articles s'afficheront en première page. Vous pouvez également ajouter une méta description à la page de la liste de blog pour un meilleur référencement :
...
<details>
<summary>Exemples de patterns pris en charge</summary>
...
<TabItem value="single" label="Auteur unique">
```
...
<TabItem value="multiple" label="Plusieurs auteurs">
```
...
<details>
  <summary>Mélanger les auteurs en ligne et les auteurs globaux</summary>
...
<details>
  <summary>Remplacer localement les auteurs globaux</summary>
...
<details>
  <summary>Localiser le fichier de configuration de l'auteur</summary>
...
Le callback `readingTime` reçoit trois paramètres : le texte du contenu du blog sous forme de chaîne de caractères, le front matter sous forme d'un enregistrement de clés de chaîne de caractères et de leurs valeurs, et la fonction de temps de lecture par défaut. Il renvoie un nombre (temps de lecture en minutes) ou `undefined` (pour désactiver le temps de lecture pour cette page).

Le temps de lecture par défaut est capable d'accepter des options supplémentaires : `wordsPerMinute` comme un nombre (par défaut : 300) et `wordBound` comme une fonction qui transforme une chaîne de caractères en booléen. Si la chaîne passée à `wordBound` doit être un mot lié (espaces, tabulations et sauts de ligne par défaut), la fonction doit retourner `true`.
...
```mdx-code-block
<Tabs>
<TabItem value="disable-per-post" label="Désactivation par article">
```
...
<TabItem value="passing-options" label="Options de transmission">
```
...
<TabItem value="using-custom-algo" label="Utilisation d'algorithmes personnalisés">
```
...
type BlogOptions = {
  feedOptions?: {
    ...
    description?: string;
    copyright: string;
    language?: string; // valeurs possibles : http://www.w3.org/TR/REC-html40/struct/dirlang.html#langcodes
    /** Permet de contrôler la construction de BlogFeedItems */
...
Vous pouvez exécuter votre site Docusaurus 2 sans page d'accueil dédiée, et utiliser la liste des articles de votre blog comme page d'index. Définissez `routeBasePath` comme étant '/' pour servir le blog via la route racine `example.com/` au lieu de la sous-route `example.com/blog/`.
...
Il y a aussi un « mode Docs-uniquement » pour ceux qui ne veulent utiliser que les Docs. Lisez le [Mode Docs uniquement](./guides/docs/docs-introduction.mdx) pour des instructions détaillées ou une explication plus élaborée de `routeBasePath`.
...
À titre d'exemple, nous hébergeons un deuxième blog [ici](/tests/blog).
