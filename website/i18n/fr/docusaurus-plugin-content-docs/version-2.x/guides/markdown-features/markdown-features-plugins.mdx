...
Si vous jouez avec le [terrain de jeu MDX](https://mdx-git-renovate-babel-monorepo-mdx.vercel.app/playground), vous remarquerez que la transpilation MDX comporte deux étapes intermédiaires : Markdown AST (MDAST), et Hypertext AST (HAST), avant d'arriver à la sortie JSX finale. Les plugins MDX sont également fournis sous deux formes :
...
La présentation ci-dessous **n'est pas** un guide complet pour la création d'un plugin, mais simplement une illustration de la manière de le faire fonctionner avec Docusaurus. Consultez la documentation [Remark](https://github.com/remarkjs/remark/blob/main/doc/plugins.md#creating-plugins) ou [Rehype](https://github.com/remarkjs/remark/blob/main/doc/plugins.md#creating-plugins) pour une explication plus approfondie de leur fonctionnement.
