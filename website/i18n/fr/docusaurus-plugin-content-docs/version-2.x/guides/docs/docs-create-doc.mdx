...
Pour en savoir plus sur [l'importation de pages partielles](../markdown-features/markdown-features-react.mdx#importing-markdown).

:::

## Front matter du Doc {#doc-front-matter}

Le [front matter](../markdown-features/markdown-features-intro.mdx#front-matter) est utilisé pour fournir des métadonnées supplémentaires pour votre page de doc. Le frontmatter est optionnel — Docusaurus pourra déduire toutes les métadonnées nécessaires sans le frontmatter. Par exemple, la fonctionnalité des [tags de doc](#doc-tags) introduite ci-dessous nécessite l'utilisation du front matter. Pour connaître tous les champs possibles, consultez [la documentation de l'API](../../api/plugins/plugin-content-docs.mdx#markdown-front-matter).

## Tags de doc {#doc-tags}

En option, vous pouvez ajouter des tags à vos pages de docs, ce qui introduit une autre dimension de catégorisation en plus de la [barre latérale des docs](./sidebar/index.mdx). Les tags sont passés dans le frontmatter comme une liste de libellés :
...
Pour en savoir plus sur toutes les [syntaxes de tableau Yaml](https://www.w3schools.io/file/yaml-arrays/).

:::

## Organisation de la structure des dossiers {#organizing-folder-structure}
...
Par défaut, l'emplacement de l'URL d'un document est son chemin de fichier relatif au dossier `docs`. Utilisez `slug` du front matter pour modifier l'URL d'un document.
...
`slug` sera ajouté au `routeBasePath` du plugin doc, qui est `/docs` par défaut. Consultez le [Mode Docs uniquement](docs-introduction.mdx#docs-only-mode) pour savoir comment supprimer la partie `/docs` de l'URL.
