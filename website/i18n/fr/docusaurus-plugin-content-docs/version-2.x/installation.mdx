...
Docusaurus est essentiellement un ensemble de [paquets](https://github.com/facebook/docusaurus/tree/main/packages) de npm.
...
- [Node.js](https://nodejs.org/en/download/) version 16.14 ou supérieure (qui peut être vérifiée en exécutant `node -v`). Vous pouvez utiliser [nvm](https://github.com/nvm-sh/nvm) pour gérer plusieurs versions de Node sur une seule machine installée.
...
Si vous configurez un nouveau site web Docusaurus pour un projet open source Facebook, utilisez le template `facebook` à la place, qui est fourni avec quelques valeurs par défaut utiles propres à Facebook :
