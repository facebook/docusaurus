...
Un plugin est une fonction qui prend deux paramètres : `context` et `options`. Il retourne un objet d'instance du plugin (ou une promesse). Vous pouvez créer des plugins en tant que fonctions ou modules. Pour plus d'informations, reportez-vous à la [section références des méthodes de plugin](../api/plugin-methods/README.mdx).
