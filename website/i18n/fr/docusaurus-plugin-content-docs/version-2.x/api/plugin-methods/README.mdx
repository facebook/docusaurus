...
async function myPlugin(context, opts) {
  return {
    ...
    configureWebpack(config, isServer, utils, content) {
      // Modifie la configuration interne de webpack. Si la valeur retournée est un Objet, elle
      // sera fusionnée dans la configuration finale en utilisant webpack-merge;
      // Si la valeur retournée est une fonction, elle recevra la configuration comme 1er argument et un drapeau isServer comme 2ème argument.
    },
    ...
    getClientModules() {
      ...
      // React ne rend l'interface utilisateur initiale.
    },
    ...
    async getTranslationFiles({content}) {
      // Renvoi des fichiers de traduction
    },
