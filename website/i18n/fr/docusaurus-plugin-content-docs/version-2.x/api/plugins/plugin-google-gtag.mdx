...
Le plugin par défaut de [Global Site Tag (gtag.js)](https://developers.google.com/tag-platform/gtagjs). Il s'agit d'un framework de balisage JavaScript et d'une API qui vous permet d'envoyer des données d'événements à Google Analytics, Google Ads et Google Marketing Platform. Cette section décrit comment configurer un site Docusaurus pour activer les balises globales pour Google Analytics.

:::tip

Vous pouvez utiliser l'outil [Google Tag Assistant](https://tagassistant.google.com/) pour vérifier si votre gtag est correctement configuré !
