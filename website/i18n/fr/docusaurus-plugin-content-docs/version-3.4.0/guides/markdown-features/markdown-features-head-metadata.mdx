...
Cette fonctionnalité s'appuie sur le composant Docusaurus [`<Head>`](./../../docusaurus-core.mdx#head). Reportez-vous vers [react-helmet](https://github.com/nfl/react-helmet) pour une documentation exhaustive.
...
Les métadonnées de description des pages Markdown peuvent être utilisées à d'autres endroits que dans les métadonnées d'entête. Par exemple, l'[index de catégorie généré](../docs/sidebar/items.mdx#generated-index-page) par le plugin docs utilise les métadonnées de description des fiches de doc.
