...
Pour définir le niveau de l'entête pour _toutes_ les pages, utilisez l'option [`themeConfig.tableOfContents`](../../api/themes/theme-configuration.mdx#table-of-contents).
...
L'option `themeConfig` s'appliquera à toutes les tables de matières du site, y compris la [table de matières en ligne](#inline-table-of-contents), mais les options du frontmatter n'affectent que la table de matières en haut à droite. Vous devez utiliser les props `minHeadingLevel` et `maxHeadingLevel` pour personnaliser chaque composant `<TOCInline />`.
...
Les entêtes Markdown dans des zones cachées apparaîtront toujours dans la table des matières. Par exemple, les entêtes dans les [`Tabs`](./markdown-features-tabs.mdx) et les [`details`](./markdown-features-intro.mdx#details) ne seront pas exclus.
...
## Exemple de section 3 {#example-section-3}
