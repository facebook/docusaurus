...
Les blocs de code sont des blocs de texte enveloppés par des chaînes de 3 backticks. You may check out [this reference](https://mdxjs.com/docs/) for the specifications of MDX.
...
```
````
Utilisez la méta-chaîne du langage correspondant pour votre bloc de code et Docusaurus récupérera automatiquement la coloration syntaxique, alimentée par [Prism React Renderer](https://github.com/FormidableLabs/prism-react-renderer).
<BrowserWindow>
...
Pour ajouter la coloration syntaxique pour n'importe lequel des autres langages [pris en charge par Prism](https://prismjs.com/#supported-languages), définissez-la dans un tableau de langages supplémentaires.

:::note

Chaque langage supplémentaire doit être un nom de composant Prism valide. Par exemple, Prism mettrait en correspondance le _langage_ `cs` avec `csharp`, mais seul `prism-csharp.js` existe en tant que _composant_, vous devez donc utiliser `additionalLanguages : ['csharp']`. Vous pouvez consulter `node_modules/prismjs/components` pour trouver tous les composants (langages) disponibles.
...
Vous pouvez vous référer aux [définitions officielles des langages de Prism](https://github.com/PrismJS/prism/tree/master/components) lorsque vous rédigez vos propres définitions de langage.
...
### Surlignage avec des commentaires {#highlighting-with-comments}

Vous pouvez utiliser des commentaires avec `highlight-next-line`, `highlight-start`, et `highlight-end` pour sélectionner quelles lignes sont surlignées.
...
/* Si vous avez un thème de surbrillance syntaxique différent pour le mode sombre. */
[data-theme='dark'] {
  /* Couleur qui convient au thème en mode sombre pour la coloration syntaxique */
...
:::
### Commentaires magiques personnalisés {#custom-magic-comments}
...
export default {
  themeConfig: {
    prism: {
      magicComments: [
        // N'oubliez pas d'étendre également le nom de la classe de mise en évidence par défaut !
        {
...
L'utilisation de CSS pour cibler la classe peut déjà faire beaucoup, mais vous pouvez débloquer tout le potentiel de cette fonctionnalité via le [swizzling](../../swizzling.mdx).
...
## Numérotation des lignes {#line-numbering}
...
## Éditeur de code interactif {#interactive-code-editor}

(Propulsé par [React Live](https://github.com/FormidableLabs/react-live))
...
type EditUrlFunction = (params: {
  // Cela ne se transforme pas en un lien (pour une bonne raison !)
...
Au lieu de mettre en place un composant dédié aux blocs de code pour la prise en charge de multi-langage, nous avons implémenté un composant [`<Tabs>`](./markdown-features-tabs.mdx) pour un usage général dans le thème classic afin que vous puissiez l'utiliser également pour d'autres scénarios sans code.
...
Si vous avez plusieurs de ces onglets de code multi-langages, et que vous voulez synchroniser la sélection entre les instances de l'onglet, reportez-vous à la section [Synchronisation des choix d'onglets](markdown-features-tabs.mdx#syncing-tab-choices).
...
Docusaurus fournit un tel utilitaire dès le départ, vous évitant d'utiliser le composant `Tabs` à chaque fois. Pour activer cette fonctionnalité, installez d'abord le paquet `@docusaurus/remark-plugin-npm2yarn` comme ci-dessus, puis dans `docusaurus.config.js`, pour les plugins où vous avez besoin de cette fonctionnalité (doc, blog, pages, etc.), enregistrez-la dans l'option `remarkPlugins`. (Consultez [la documentation de configuration](../../api/plugins/plugin-content-docs.mdx#ex-config) pour plus de détails sur le format de configuration)
...
## Utilisation dans JSX {#usage-in-jsx}
...
Bien que déconseillé, vous pouvez également passer dans une prop `metastring` avec `metastring='{1-2} title="/src/components/HelloCodeTitle.js"' showLineNumbers'`, qui est la façon dont les blocs de code Markdown sont gérés sous le capot. Cependant, nous vous recommandons d'utiliser [des commentaires pour mettre en surbrillance les lignes](#highlighting-with-comments).

Comme [indiqué précédemment](#using-jsx-markup), la coloration syntaxique n'est appliquée que lorsque les enfants sont une chaîne de caractères simple.
