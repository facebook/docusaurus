...
Docusaurus se compose d'un ensemble de [paquets](https://github.com/facebook/docusaurus/tree/main/packages) de npm .

:::tip

Utilisez la **[Procédure accélérée](introduction.mdx#fast-track)** pour comprendre Docusaurus en **5 minutes ⏱** !
...
- [Node.js](https://nodejs.org/en/download/) version 18.0 ou supérieure (qui peut être vérifiée en exécutant `node -v`). You can use [nvm](https://github.com/nvm-sh/nvm) to manage multiple Node.js versions on a single machine.
...
The easiest way to install Docusaurus is to use the [`create-docusaurus`](./api/misc/create-docusaurus.mdx) command line tool that helps you scaffold a skeleton Docusaurus website. Vous pouvez exécuter cette commande n'importe où dans un nouveau dépôt vide ou dans un dépôt existant, elle créera un nouveau répertoire contenant les fichiers de structure.
...
Exécutez `npx create-docusaurus@latest --help`, ou consultez sa [Documentation API](./api/misc/create-docusaurus.mdx) pour plus d'informations sur tous les drapeaux disponibles.
...
- `/src/` - Fichiers de non-documentation comme les pages ou les composants React personnalisés. Vous n'êtes pas obligé de placer vos fichiers de non-documentation ici, mais les placer dans un répertoire centralisé permet de les spécifier plus facilement au cas où vous auriez besoin de faire une sorte de vérification/traitement
  - `/src/pages` - Tous les fichiers JSX/TSX/MDX de ce répertoire seront convertis en page de site. Vous trouverez plus de détails dans le [guide des pages](guides/creating-pages.mdx)
...
Si vous utilisez un hébergeur comme Netlify ou Vercel, vous devrez changer le `répertoire de base` du site à l'endroit où se trouve votre racine Docusaurus. Dans ce cas, ce serait `./website`. En savoir plus sur la configuration des commandes ignorées dans les [docs de déploiement](./deployment.mdx#deploying-to-netlify).
...
## Mise à jour de votre version de Docusaurus {#updating-your-docusaurus-version}
...
Utilisez de nouvelles fonctionnalités inédites de Docusaurus avec la balise [`@canary` npm dist tag](/community/canary)
