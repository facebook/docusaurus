...
Nous maintenons une [liste de plugins officiels](./api/plugins/overview.mdx), mais la communauté a également créé des [s plugins non officiels](/community/resources#community-plugins). Si vous souhaitez ajouter une fonctionnalité quelconque : génération automatique de pages de documentation, exécution de scripts personnalisés, intégration d'autres services... n'oubliez pas de consulter la liste : quelqu'un l'a peut-être implémentée pour vous !

Si vous vous sentez énergique, vous pouvez également lire [le guide du plugin](./advanced/plugins.mdx) ou [les références de la méthode du plugin](./api/plugin-methods/README.mdx) pour savoir comment faire un plugin vous-même.
...
## Utiliser les presets {#using-presets}
...
- [`@docusaurus/plugin-google-analytics`](./api/plugins/plugin-google-analytics.mdx) (**déprécié**)
