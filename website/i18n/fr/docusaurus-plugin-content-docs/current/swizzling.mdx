...
Ceci copiera l'implémentation du composant `<Footer />` actuel dans le répertoire `src/theme` de votre site. Docusaurus utilisera désormais cette copie de composant `<Footer>` au lieu de l'originale. Vous êtes alors libre de réimplémenter complètement le composant `<Footer>`.
...
- **Sûr** : ce composant peut être swizzlé en toute sécurité. Son API publique est considérée comme stable et aucune modification destructrice ne devrait se produire dans une **version majeure** du thème.
- **Non sûr** : ce composant est un détail d'implémentation de thème. Il est risqué de le swizzler ; des modifications destructrices pourraient se produire dans une **version mineure** du thème.
- **Interdit** : la commande swizzle CLI vous empêchera de swizzler ce composant, car il n'est pas du tout conçu pour cela.
...
Ne soyez pas trop **effrayé par l'utilisation de composants non sécurisés** : gardez simplement à l'esprit que des **modifications destructrices** pourraient se produire, et que vous pourriez avoir besoin de mettre à jour vos personnalisations manuellement lors de mises à jour mineures.
...
4. **Lisez le [code source du thème](https://github.com/facebook/docusaurus/tree/main/packages/docusaurus-theme-classic/src/theme)** et utilisez la recherche à bon escient.
...
Nous souhaitons aussi mieux comprendre vos cas de personnalisation les plus avancés, alors n'hésitez pas à [**nous les partager**](https://github.com/facebook/docusaurus/discussions/5468).
...
**Plus c'est petit, mieux c'est.** Si le swizzling est inévitable, essayez de swizzler seulement la partie pertinente, de façon à n'avoir à maintenir vous-même que le moins de code possible. Swizzler un petit composant présentera souvent moins de risques de **modifications destructrices** lors d'une mise à jour.

[L'enveloppement](#wrapping) est aussi une alternative bien plus sûre que [l'éjection](#ejecting).

:::

## Enveloppez votre site avec `<Root>` {#wrapper-your-site-with-root}
