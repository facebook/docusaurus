...
`contentLoaded` sera appelé _après_ que `loadContent` soit fait. La valeur retournée de `loadContent()` sera passée à `contentLoaded` comme `content (contenu)`.
...
Ces données deviennent accessibles pour votre code client/thème, via les [`useGlobalData`](../../docusaurus-core.mdx#useGlobalData) et [`usePluginData`](../../docusaurus-core.mdx#usePluginData).
...
### Fusionnez la stratégie {#merge-strategy}
...
Lisez la documentation de la stratégie de fusion de [webpack-merge](https://github.com/survivejs/webpack-merge#merging-with-strategies) pour plus de détails.

### Configuration du serveur de développement {#configuring-dev-server}
...
Retourne un tableau de chemins vers les [modules client](../../advanced/client.mdx#client-modules) qui doivent être importés dans le paquet client.
