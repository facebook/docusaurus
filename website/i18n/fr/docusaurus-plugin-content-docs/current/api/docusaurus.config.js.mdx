...
Reportez-vous à la [**Configuration**](../configuration.mdx) du guide de démarrage pour des exemples.
...
Reportez-vous à la [**Syntaxe de déclaration de `docusaurus.config.js`**](../configuration.mdx#syntax-to-declare-docusaurus-config) pour une liste plus exhaustive d'exemples et d'explications.
...
Reportez-vous au [guide de déploiement](../deployment.mdx) et [slorber/trailing-slash-guide](https://github.com/slorber/trailing-slash-guide) pour choisir le paramètre approprié.
...
- `localeConfigs` : Options individuelles pour chaque locale.
  - `label` : Libellé affiché pour cette locale dans le menu déroulant des locales.
  - `direction` : `ltr` (par défaut) ou `rtl` (pour les [langues de droite à gauche](https://developer.mozilla.org/en-US/docs/Glossary/rtl) comme le persan, l'arabe, l'hébreu, etc.). Utilisé pour sélectionner le méta attribut CSS et HTML de la locale.
  - `htmlLang` : balise de langue BCP 47 à utiliser dans `<html lang="...">` (ou n'importe quel autre nom de balise du DOM) et dans `<link ... hreflang="...">`
  - `calendar` : le [calendrier](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/Locale/calendar) utilisé pour calculer l'ère de la date. Remarquez qu'il ne contrôle pas la chaîne de caractères affichée : `MM/DD/YYYY` et `DD/MM/YYYY` sont tous deux `grégoriens`. Pour choisir le format (`DD/MM/YYYY` ou `MM/DD/YYYY`), définissez votre nom de locale comme `fr-FR` ou `en-US` (`en` signifie `en-US`).
...
Voir [les références de méthodes de plugin](./plugin-methods/README.mdx) pour la forme d'un `PluginModule`.
...
| `remarkRehypeOptions` | `object`                      | `undefined`                                             | Permet de passer des [options personnalisées `remark-rehype`](https://github.com/remarkjs/remark-rehype#options).                                                                                                                                  |
...
Un tableau de [modules client](../advanced/client.mdx#client-modules) à charger globalement sur votre site .
...
Si vous avez une [stratégie de sécurité du contenu](https://developer.mozilla.org/en-US/docs/Web/HTTP/CSP) stricte, vous devriez plutôt la désactiver.
