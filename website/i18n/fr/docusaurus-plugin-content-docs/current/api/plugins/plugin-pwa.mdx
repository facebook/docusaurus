...
Créer un [manifest PWA](https://web.dev/add-manifest/) dans `./static/manifest.json`.
...
![Un extrait d'écran du processus d'installation. Un bouton apparaît dans la barre d'adresse du navigateur, qui affiche une boîte de dialogue demandant "installer cette application ?" lorsqu'il est cliqué. Après avoir cliqué sur le bouton "Installer", une nouvelle application est ouverte dans le système d'exploitation, s'ouvrant sur la page d'accueil de Docusaurus.](/img/pwa_install.gif)
...
La page [workbox-precaching](https://developers.google.com/web/tools/workbox/modules/workbox-precaching) explique le principe :
...
Il n'est pas possible de détecter de manière fiable si une page est rendue comme une PWA.
...
![Un extrait d'écran du processus de rechargement. Une boîte d'alerte apparaît en bas à droite de la fenêtre, disant "Nouveau contenu disponible". Après avoir cliqué sur le bouton "Actualiser", le titre principal de la page passe de "Introduction" à "PWA :))".](/img/pwa_reload.gif)
