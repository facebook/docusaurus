...
export default function ImportAttributesTsxTests(): React.JSX.Element {
  return (
    <Layout>
      <main className="container margin-vert--lg">
        <Heading as="h1">Import attributes tests (TSX)</Heading>
        <p>
          See also the{' '}
          <Link to="/tests/pages/import-attributes">MDX version</Link> of these
          tests.
        </p>

        <Heading as="h2">JSON import</Heading>
        <pre>{JSON.stringify(jsonData)}</pre>

        <Heading as="h2">Text imports</Heading>
        <p>
          Modules imported with <code>{"with {type: 'text'}"}</code> should be
          the raw file content, not processed by any loader. These tests fail
          the build otherwise.
        </p>
        <TextImportTest
          name="JS"
