...
function processMetadata(file) {
  ...
  // We don't have these by default.
  let langPart = '',
    versionPart = '';

  if (env.translation.enabled && language) {
    langPart = `${language}/`;
  }
...
function generateMetadataDocs() {
  ...
  files.forEach(file => {
    ...
    if (extension === '.md' || extension === '.markdown') {
      ...
      enabledLanguages.map(currentLanguage => {
        ...
        if (baseMetadata['next'])
          baseMetadata['next'] = prefix + baseMetadata['next'].toString();
        if (baseMetadata['previous'])
          baseMetadata['previous'] =
            prefix + baseMetadata['previous'].toString();
