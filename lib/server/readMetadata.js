...
function generateMetadataDocs() {
  ...
  const enabledLanguages = env.translation.enabledLanguageTags();
  ...
  files.forEach(file => {
    ...
    if (extension === '.md' || extension === '.markdown') {
      ...
      /**
       * This code does not handle an undefined language
       * This and the 'processMetadata' function presume any file in the
       * root of docs is the English Translation.
       *
       * This should not be the case. We have no guarantees what language the users documents are in.
       *
       * Think of it this way. If the untranslated files are in English, how can we have an English translation?
       * English != English ?
       */
...
function generateMetadataBlog() {
  const metadataFile = path.join(__dirname, '..', 'core', 'MetadataBlog.js');

  // Remove it if it exists
  if (fs.existsSync(metadataFile)) {
    fs.removeSync(metadataFile);
  }
