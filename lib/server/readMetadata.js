...
function processMetadata(file) {
  ...
  const separator = getDocsPath() + '/';

  metadata.source = file.split(separator)[1];

  let sourceDir = path.dirname(metadata.source);

  if (sourceDir.startsWith('.') && sourceDir.length === 1) {
    sourceDir = '';
  } else {
    sourceDir = sourceDir + '/';
  }
  ...
  if (!metadata.id) {
    metadata.id = sourceDir + path.basename(file, path.extname(file));
  } else {
    if (metadata.id.includes('/')) {
        throw new Error('Document id cannot include "/".');
    }

    metadata.id = sourceDir + metadata.id;
  }
...
function generateMetadataDocs() {
  ...
  let files = glob.sync(CWD + '/../' + getDocsPath() + '/**/*.{md,markdown}');
  files.forEach(file => {
