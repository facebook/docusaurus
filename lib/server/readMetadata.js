...
function processMetadata(file) {
  ...
  let languagePrefix = language ? language + "-" : ""; 
  ...
  // If we are in a language folder
  if (language) {
    metadata.id = languagePrefix + metadata.id;
  ...
  if (order[id]) {
    ...
    if (order[id].next) {
      ...
      metadata.next = languagePrefix + order[id].next;
    }
    if (order[id].previous) {
      metadata.previous_id = order[id].previous;
      metadata.previous = languagePrefix + order[id].previous;
    }
  }
...
function generateMetadataDocs() {
  ...
  const languagesToGenerate = env.translation.enabledLanguages().map(language => language.tag);
  console.log(languagesToGenerate);
  ...
  files.forEach(file => {
    ...
    if (extension === '.md' || extension === '.markdown') {
      ...
      languagesToGenerate.map(currentLanguage => {
          var prefix = currentLanguage + "-"

          let baseMetadata = Object.assign({}, metadata);
          baseMetadata['id'] = prefix + baseMetadata['id']
          if (baseMetadata['permalink'])
            baseMetadata['permalink'] = baseMetadata['permalink']
              .toString()
              .replace(/^docs\//, 'docs/' + currentLanguage + '/');
          if (baseMetadata['next']) baseMetadata['next'] = prefix + baseMetadata['next'].toString();
          if (baseMetadata['previous']) baseMetadata['previous'] = prefix + baseMetadata['previous'].toString();
  ...
  files.forEach(file => {
    ...
    if (languagesToGenerate.indexOf(language) === -1) {
      return;
    }
