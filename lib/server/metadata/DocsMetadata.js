...
class DocsMetadata {
  ...
  populate() {
    ...
    files.forEach(file => {
      if (!translation.getFileLanguage(file, translatedDir)) {
        return;
      }
    ...
    Object.keys(metadatas).forEach(docId => {
      ['next', 'previous'].forEach(key => {
        ...
        const {title = null} = metadatas[siblingId] || {};

        metadatas[docId][`${key}_title`] = title;
      });
    });
  ...
  processMetadata(file, refDir) {
    ...
    const language = translation.getFileLanguage(file, refDir) || 'en';
