...
function readCategories(sidebar, requestedLanguage) {
  ...
  Object.keys(Metadata).forEach(id => {
    ...
    // Ignore any that don't belong in this sidebar.
    if (metadata.sidebar != sidebar) return;
    if (metadata.language != requestedLanguage) return;

    metadatas.push(metadata);
  });

  // Build a hashmap of article_id -> metadata for this language
  const articles = {};
