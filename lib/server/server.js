...
function execute(port) {
  ...
  function getPossibleLanguageTags(url)
  {
    console.log(url);
    ...
    const translatedStrings = translation['languages'];
    let validLangtags = translatedStrings.map((lang) => (lang.tag));
  ...
  app.get(/docs\/.*html$/, (req, res, next) => {
    ...
    const language = getPossibleLanguageTags(req.path);

    var docpath = siteConfig.customDocsPath ? siteConfig.customDocsPath : 'docs';
    ...
    if (metadata.original_id) {
      ...
      if (metadata.language) {
        file = CWD + '/translated_docs/' + metadata.language + '/' + metadata.source;
      } else {
  ...
  app.get('*.html', (req, res, next) => {
    ...
    let univeralFile = CWD + '/pages/' + file;
    
    // If it's a valid language, we may need a backup link
    if (language != null) {
      universalFile = universalFile.replace('/' + language + '/', '/');
    }
    
    // check for the specified file
    let validatedFile = fs.existsSync(userFile) ? userFile : false;
    // or the universal file, if it's different
    if (!validatedFile && userFile != universalFile) {
      validatedFile = fs.existsSync(universalFile) ? universalFile : false;
    }
    ...
    let parts = validatedFile.split('pages/');
    ...
    fs.copySync(validatedFile, tempFile);
