...
function execute(port) {
  ...
  function reloadMetadataBlog() {
    ...
    // Delete is inside generate function
  ...
  app.get(/blog\/.*html$/, (req, res) => {
     ...
     * - Neil
     *
     * Okay, so this is a bit insane.
     * On a request for a single entry, we render all pages of the blog posts.
     * posts. Then after this we decide which one to send back.
     *
     * And if it was actually a specific page. We request it from the actual file, and discard the listings.
     *
     * A better way could be to work out the filename from the url requested and then return the markdown file
     *
     * ~/page2/ -> We start counting at 1, there is no zero
     * ~/2017/05/06/jest-20-delightful-testing-multi-project-runner.html -> We replace all the slashes with dashes
     *
     * For the listing, we would have the page numer, and can work out the index of the metadata we need to read.
     * For the blog post, we have the exact name of the file it needs.
     *
     * This is out of the scope for this PR, so just documenting my findings
     *
     * Maybe there is a reason it is this way, but I can't find it ATM
     */
  ...
  app.get(/main\.css$/, (req, res) => {
    var content = render.css.renderMainCss();
