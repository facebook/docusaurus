...
function execute(port) {
  ...
  app.get(/docs\/.*html$/, (req, res, next) => {
    console.log("okay, request recieved.");
  ...
  function redirect_page(req, res) {
    ...
    let requestURL = 'http://localhost:' + port + req.path + slash + 'index.html'
    console.log('request url: ' + requestURL + " and req.path: " + req.path);
    request.get(
      requestURL,
      (err, response, body) => {
        if (!err) {
 ...
 */
  function redirect_page_noHTML(path, res) {
    ...
    console.log('got to redirect_noHTML: ' + requestURL);
    request.get(
      requestURL,
      (err, response, body) => {
        if (!err) {
