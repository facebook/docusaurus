...
const parseCss = css => {
  ...
  if (
    ...
    console.error(
      `${chalk.yellow(
        'Warning: Missing color configuration. Make sure siteConfig.colors includes primaryColor and secondaryColor fields'
      )}`
    );
  }
  ...
  variables['codeColor'] = color(siteConfig.colors.primaryColor)
    .alpha(0.07)
    .string();
...
const renderMainCss = () => {
  ...
  let websiteCssFiles = glob.sync(join(CWD, 'static', '**', '*.css'), {
    nodir: true,
  });
