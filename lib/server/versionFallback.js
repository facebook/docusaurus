...
const utilsMetadata = require('./utilsMetadata');
...
files.forEach(file => {
  ...
  const res = utilsMetadata.extractMetadata(fs.readFileSync(file, 'utf8'));
...
function processVersionMetadata(file, version, useVersion, language) {
  const metadata = utilsMetadata.extractMetadata(fs.readFileSync(file, 'utf8'))
    .metadata;
