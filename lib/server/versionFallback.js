...
function docVersion(id, req_version) {
  ...
  throw new Error(
    `No document with id '${id}' available for use in the requested version, ${req_version}, of the website. Verify that all version files are correct.`
  );
}
...
function sidebarVersion(req_version) {
  ...
  console.error(`No sidebar file available to use for version ${req_version}. Verify that all version files are correct.`);
  throw new Error(
