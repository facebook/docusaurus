...
files.forEach(file => {
  ...
  if (!metadata.original_id) {
    console.error(`No 'original_id' field found in the metadata for ${file}.`);
    process.exit(1);
  }
  ...
  if (!metadata.id) {
    ...
    console.error(`The 'id' field in the metadata for ${file} is missing the expected 'version-XX-' prefix.`);
    process.exit(1);
  }
