...
const cssnanoOptions = require('../server/cssnanoOptions');
...
describe('Build files', () => {
  ...
  test('Concatenated CSS files', async function() {
    ...
    const minifiedInputs = await Promise.all(
      inputFileContents.map(fileContent =>
        cssnano.process(fileContent, cssnanoOptions)
      )
    );

    minifiedInputs.forEach(fileContent => {
      expect(outputFileContent).toContain(fileContent.css);
    });
  });
