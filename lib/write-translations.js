...
const FILE_TRANSLATIONS = '/i18n/en.json';
...
if (fs.existsSync(CWD + '/i18n/en.json')) {
  currentTranslations = JSON.parse(
    fs.readFileSync(CWD + FILE_TRANSLATIONS, 'utf8')
  );
}
      ...
      traverse(ast, {
        enter(path) {
          if (
            ...
            const text = prepareTranslationText(path.node.children[0].value);
            ...
            switch (path.node.arguments.length) {
              case 2:
                if (path.node.arguments[1].type !== 'StringLiteral') {
                  console.warn(
                    `Skipping "translate" call in ${file} at line ${
                      path.node.loc.start.line
                    } when writing to ${FILE_TRANSLATIONS}. To enable autogeneration you must pass description as ` +
                      `a string literal. Expressions and templates are ignored. You can still add desired strings ` +
                      `to ${FILE_TRANSLATIONS} with your hands.`
                  );

                  break;
                }
                description = prepareTranslationText(
              ...
              case 1:
                if (path.node.arguments[0].type !== 'StringLiteral') {
                  console.warn(
                    `Skipping "translate" call in ${file} at line ${
                      path.node.loc.start.line
                    } when writing to ${FILE_TRANSLATIONS}. To enable autogeneration you must pass the string you ` +
                      `want to translate as a string literal. Expressions and templates are ignored. You can still ` +
                      `add desired strings to ${FILE_TRANSLATIONS} with your hands.`
                  );

                  break;
                }

                const text = prepareTranslationText(
              ...
              default:
                console.warn(
                  `Skipping "translate" call in ${file} at line ${
                    path.node.loc.start.line
                  } when writing to ${FILE_TRANSLATIONS}. To enable autogeneration you must pass only 1 or 2 ` +
                    `arguments that must be string literals. Expressions and templates are ignored. You can still ` +
                    `add desired strings to ${FILE_TRANSLATIONS} with your hands.`
                );
            }
          }
