...
class DocsLayout extends React.Component {
  ...
  render() {
    ...
    const config = this.props.siteConfig;
    const customLayouts = siteConfig.layouts;
    let DocComponent = Doc;
    if (customLayouts) {
      if (customLayouts.default) {
        DocComponent = customLayouts.default({
          React,
          MarkdownBlock,
        });
      } else if (metadata.layout && customLayouts[metadata.layout]) {
        DocComponent = customLayouts[metadata.layout]({
          React,
          MarkdownBlock,
        });
      }
