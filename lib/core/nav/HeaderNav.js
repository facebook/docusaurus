  ...
  render() {
    ...
    const enabledLanguages = env.translation
      ...
      .map(lang => {
        ...
        let href = urljoin(siteConfig.baseUrl, lang.tag);
        if (
          this.props.current &&
          ...
          href = urljoin(siteConfig.baseUrl, lang.tag, this.props.current.id);
        }
        return (
          <li key={lang.tag}>
  ...
  makeLinks(link) {
    ...
    if (link.search && this.props.config.algolia) {
      ...
      if (fs.existsSync(CWD + '/pages/en/' + link.page + '.js')) {
        href = urljoin(siteConfig.baseUrl, language, link.page, extension);
      } else {
        href = urljoin(siteConfig.baseUrl, link.page, extension);
      }
    } else if (link.href) {
      ...
      href = urljoin(this.props.baseUrl, 'blog');
    }
