...
const getLanguageName = (langTag) => {
  let match = env.translation.enabledLanguages().filter(lang => lang.tag == langTag)[0];
...
class LanguageDropDown extends React.Component {
  render() {
    let currentLanguage = getLanguageName(this.props.language);
    ...
    const languageOptions = env.translation
      .enabledLanguages()
      ...
      <span>
        <li key="languages">
          ...
          <div id="languages-dropdown" className="hide">
            <ul id="languages-dropdown-items">{languageOptions}</ul>
          </div>
        </li>
        <script
