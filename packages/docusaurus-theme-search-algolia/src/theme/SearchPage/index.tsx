...
function SearchPageContent() {
  ...
  // check theme settings for contextualFacets bool
    ...
    ({ results: { query, hits, page, nbHits, nbPages } }) => {
      ...
      const items = hits.map(
        ({
          ...
          // highlightResult hierarchy values are converted to array
          // this provides both title(potential) and breadcrumbs
          const hierarchyValues = Object.keys(hierarchy).map((key) => {
            return hierarchy[key].value;
          });

          // determine item title
          const determineTitle = (title, hierarchy) => {
            // if explicitly defined title, use it
            if (title) {
              return title;
            }
            //else use highest level hierarchy value
            else if (hierarchy) {
              const title = hierarchyValues.pop();
          ...
          const determineSummary = (description, snippet) => {
            if (description) {
              return description;
            } else if (snippet.content) {
              return `${sanitizeValue(snippet.content.value)}...`;
            } else {
              return "";
            }
          };
          ...
          return {
            title: determineTitle(title, hierarchy),
            url: processSearchResultUrl(url),
            summary: determineSummary(description, snippet),
            breadcrumbs: hierarchyValues,
          };
        }
      );
  ...
  const makeSearch = useEvent((page = 0) => {
    // check theme settings for contextualFacets bool before adding facets
    if (contextualSearch) {
