...
export default async function render(locals: any): Promise<string> {
  try {
...
async function doRender(locals: any) {
  const {
    routesLocation,
    headTags,
    preBodyTags,
  ...
  const stylesheets = (bundles.css || []).map((b: any) => b.file);
  const scripts = (bundles.js || []).map((b: any) => b.file);
