...
const itemWithType = (type: string | Joi.StringSchema) =>
  Joi.object({type}).unknown();

const sidebarItemSchema: Joi.Schema<SidebarItemConfig> = Joi.object()
  .when('.', {
    switch: [
      {is: itemWithType('link'), then: sidebarItemLinkSchema},
      {
        is: itemWithType(Joi.string().valid('doc', 'ref').required()),
        then: sidebarItemDocSchema,
      },
      {is: itemWithType('autogenerated'), then: sidebarItemAutogeneratedSchema},
      {is: itemWithType('category'), then: sidebarItemCategorySchema},
      {
        is: itemWithType('subcategory'),
      ...
      {
        is: itemWithType(Joi.string().required()),
        then: Joi.forbidden().messages({
