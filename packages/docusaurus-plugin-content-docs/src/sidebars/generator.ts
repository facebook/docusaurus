 ...
 */
function getAutogenDocs({item}: SidebarItemsGeneratorArgs) {
  function isInAutogeneratedDir(doc: SidebarItemsGeneratorDoc) {
    return (
      // Doc at the root of the autogenerated sidebar dir
      doc.sourceDirName === item.dirName ||
  ...
  return (allDocs: SidebarItemsGeneratorDoc[]): SidebarItemsGeneratorDoc[] => {
 ...
 */
function treeify({item}: SidebarItemsGeneratorArgs) {
  ...
  function getRelativeBreadcrumb(doc: SidebarItemsGeneratorDoc): string[] {
    return item.dirName === doc.sourceDirName
      ? []
      : doc.sourceDirName
          .replace(addTrailingSlash(item.dirName), '')
          .split(BreadcrumbSeparator);
  }
  return (docs: SidebarItemsGeneratorDoc[]): Dir => {
    const treeRoot: Dir = {};
    docs.forEach((doc) => {
...
function generateSidebar({
    ...
    return {
      ...
      items: await Promise.all(
        Object.entries(dir).map(([key, content]) =>
          dirToItem(content, key, `${relativePath}/${key}`),
        ),
      ),
    };
  }
  async function dirToItem(
    ...
    relativePath: string, // `dir`'s full path relative to the autogen dir.
  ): Promise<WithPosition<SidebarItem>> {
    return dir
      ? createCategoryItem(dir, relativePath, itemKey)
      : createDocItem(itemKey);
  }
  return (fsModel: Dir): Promise<WithPosition<SidebarItem>[]> =>
    Promise.all(
...
const DefaultSidebarItemsGenerator: SidebarItemsGenerator = async (args) => {
  // TODO: the whole code is designed for pipeline operator
  const docs = getAutogenDocs(args)(args.docs);
  const fsModel = treeify(args)(docs);
  const sidebarWithPosition = await generateSidebar(args)(fsModel);
